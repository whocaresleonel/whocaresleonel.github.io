var Godot = ( () => {
    var _scriptName = typeof document != "undefined" ? document.currentScript?.src : 0[0];
    return function(moduleArg={}) {
        ...
        (ENVIRONMENT_IS_WEB || ENVIRONMENT_IS_WORKER) && (ENVIRONMENT_IS_WORKER ? scriptDirectory = self.location.href : typeof document != "undefined" && document.currentScript && (scriptDirectory = document.currentScript.src),
        _scriptName && (scriptDirectory = _scriptName),
        scriptDirectory.startsWith("blob:") ? scriptDirectory = "" : scriptDirectory = scriptDirectory.substr(0, scriptDirectory.replace(/[?#].*/, "").lastIndexOf("/") + 1),
        ...
        }).then(response => response.ok ? response.arrayBuffer() : Promise.reject(new Error(response.status + " : " + response.url)))),
        ...
        function removeRunDependency() {
            if (runDependencies--,
            Module.monitorRunDependencies?.(runDependencies),
            runDependencies == 0 && (runDependencyWatcher !== null && (clearInterval(runDependencyWatcher),
            runDependencyWatcher = null),
        ...
        function findWasmBinary() {
            ...
            return isDataURI(f) ? f : locateFile(f)
        }
        ...
        function getBinaryPromise(binaryFile) {
            return wasmBinary ? Promise.resolve().then( () => getBinarySync(binaryFile)) : readAsync(binaryFile).then(response => new Uint8Array(response), () => getBinarySync(binaryFile))
        }
        ...
        function instantiateAsync(binary, binaryFile, imports, callback) {
            return !binary && typeof WebAssembly.instantiateStreaming == "function" && !isDataURI(binaryFile) && typeof fetch == "function" ? fetch(binaryFile, {
                credentials: "same-origin"
            }).then(response => {
                var result = WebAssembly.instantiateStreaming(response, imports);
                return result.then(callback, function(reason) {
                    return err(`wasm streaming compile failed: ${reason}`),
                    err("falling back to ArrayBuffer instantiation"),
                    instantiateArrayBuffer(binaryFile, imports, callback)
                })
            }
            ) : instantiateArrayBuffer(binaryFile, imports, callback)
        }
        ...
        function createWasm() {
            ...
            function receiveInstance(instance) {
                return wasmExports = instance.exports,
            ...
            return wasmBinaryFile || (wasmBinaryFile = findWasmBinary()),
        ...
        var getWasmTableEntry = funcPtr => wasmTable.get(funcPtr)
          , ___call_sighandler = (fp, sig) => getWasmTableEntry(fp)(sig)
          , PATH = {
            ...
            normalize: path => {
                ...
                return path = PATH.normalizeArray(path.split("/").filter(p => !!p), !isAbsolute).join("/"),
          ...
          , PATH_FS = {
            resolve: (...args) => {
                ...
                return resolvedPath = PATH.normalizeArray(resolvedPath.split("/").filter(p => !!p), !resolvedAbsolute).join("/"),
            ...
            relative: (from, to) => {
                ...
                function trim(arr) {
                    for (var end, start = 0; start < arr.length; start++)
                        if (arr[start] !== "")
                            break;
          ...
          , UTF8Decoder = typeof TextDecoder != "undefined" ? new TextDecoder : 0[0]
          , UTF8ArrayToString = (heapOrArray, idx, maxBytesToRead) => {
            for (var str, u0, u1, u2, ch, endIdx = idx + maxBytesToRead, endPtr = idx; heapOrArray[endPtr] && !(endPtr >= endIdx); )
                ++endPtr;
            if (endPtr - idx > 16 && heapOrArray.buffer && UTF8Decoder)
                return UTF8Decoder.decode(heapOrArray.subarray(idx, endPtr));
            for (str = ""; idx < endPtr; ) {
                if (u0 = heapOrArray[idx++],
                !(u0 & 128)) {
                    str += String.fromCharCode(u0);
                    continue
                }
                if (u1 = heapOrArray[idx++] & 63,
                (u0 & 224) == 192) {
                    str += String.fromCharCode((u0 & 31) << 6 | u1);
                    continue
                }
                u2 = heapOrArray[idx++] & 63,
                (u0 & 240) == 224 ? u0 = (u0 & 15) << 12 | u1 << 6 | u2 : u0 = (u0 & 7) << 18 | u1 << 12 | u2 << 6 | heapOrArray[idx++] & 63,
                u0 < 65536 ? str += String.fromCharCode(u0) : (ch = u0 - 65536,
          ...
          , lengthBytesUTF8 = str => {
            for (var c, len = 0, i = 0; i < str.length; ++i)
                c = str.charCodeAt(i),
                c <= 127 ? len++ : c <= 2047 ? len += 2 : c >= 55296 && c <= 57343 ? (len += 4,
                ++i) : len += 3;
            return len
        }
          , stringToUTF8Array = (str, heap, outIdx, maxBytesToWrite) => {
            if (!(maxBytesToWrite > 0))
                return 0;
            for (var u, u1, startIdx = outIdx, endIdx = outIdx + maxBytesToWrite - 1, i = 0; i < str.length; ++i)
                if (u = str.charCodeAt(i),
                u >= 55296 && u <= 57343 && (u1 = str.charCodeAt(++i),
        ...
        function intArrayFromString(stringy, dontAddNull, length) {
            ...
            return dontAddNull && (u8array.length = numBytesWritten),
            u8array
        }
        var FS_stdin_getChar = () => {
            if (!FS_stdin_getChar_buffer.length) {
                var result = null;
                if (typeof window != "undefined" && typeof window.prompt == "function" && (result = window.prompt("Input: "),
                result !== null && (result += `
`)),
                !result)
                    return null;
          ...
          , TTY = {
            ...
            stream_ops: {
                ...
                read(stream, buffer, offset, length) {
                    ...
                    return bytesRead && (stream.node.timestamp = Date.now()),
                    bytesRead
                },
                write(stream, buffer, offset, length) {
                    ...
                    return length && (stream.node.timestamp = Date.now()),
                    i
                }
            },
            default_tty_ops: {
                ...
                put_char(tty, val) {
                    val === null || val === 10 ? (out(UTF8ArrayToString(tty.output, 0)),
                    tty.output = []) : val != 0 && tty.output.push(val)
                },
                fsync(tty) {
                    tty.output && tty.output.length > 0 && (out(UTF8ArrayToString(tty.output, 0)),
                    tty.output = [])
                },
                ioctl_tcgets() {
                    return {
            ...
            default_tty1_ops: {
                put_char(tty, val) {
                    val === null || val === 10 ? (err(UTF8ArrayToString(tty.output, 0)),
                    tty.output = []) : val != 0 && tty.output.push(val)
                },
                fsync(tty) {
                    tty.output && tty.output.length > 0 && (err(UTF8ArrayToString(tty.output, 0)),
                    tty.output = [])
                }
            }
        }
          ...
          , MEMFS = {
            ...
            createNode(parent, name, mode, dev) {
                ...
                return FS.isDir(node.mode) ? (node.node_ops = MEMFS.ops_table.dir.node,
                node.stream_ops = MEMFS.ops_table.dir.stream,
                node.contents = {}) : FS.isFile(node.mode) ? (node.node_ops = MEMFS.ops_table.file.node,
                node.stream_ops = MEMFS.ops_table.file.stream,
                node.usedBytes = 0,
                node.contents = null) : FS.isLink(node.mode) ? (node.node_ops = MEMFS.ops_table.link.node,
                node.stream_ops = MEMFS.ops_table.link.stream) : FS.isChrdev(node.mode) && (node.node_ops = MEMFS.ops_table.chrdev.node,
            ...
            expandFileStorage(node, newCapacity) {
                ...
                node.usedBytes > 0 && node.contents.set(oldContents.subarray(0, node.usedBytes), 0)
            },
            ...
            node_ops: {
                getattr(node) {
                    var attr = {};
                    return attr.dev = FS.isChrdev(node.mode) ? node.id : 1,
                    attr.ino = node.id,
                    attr.mode = node.mode,
                    attr.nlink = 1,
                    attr.uid = 0,
                ...
                setattr(node, attr) {
                    attr.mode !== 0[0] && (node.mode = attr.mode),
                    attr.timestamp !== 0[0] && (node.timestamp = attr.timestamp),
                    attr.size !== 0[0] && MEMFS.resizeFileStorage(node, attr.size)
                },
                lookup() {
                ...
                rmdir(parent, name) {
                    var i, node = FS.lookupNode(parent, name);
                ...
                readdir(node) {
                    var key, entries = [".", ".."];
                    for (key of Object.keys(node.contents))
                        entries.push(key);
                    return entries
                },
                symlink(parent, newname, oldpath) {
                    var node = MEMFS.createNode(parent, newname, 511 | 40960, 0);
                    return node.link = oldpath,
                    node
                },
                readlink(node) {
            ...
            stream_ops: {
                read(stream, buffer, offset, length, position) {
                    var i, contents = stream.node.contents;
                ...
                write(stream, buffer, offset, length, position, canOwn) {
                    if (buffer.buffer === HEAP8.buffer && (canOwn = !1),
                    !length)
                        return 0;
                    var i, node = stream.node;
                    if (node.timestamp = Date.now(),
                    buffer.subarray && (!node.contents || node.contents.subarray)) {
                        if (canOwn)
                            return node.contents = buffer.subarray(offset, offset + length),
                            node.usedBytes = length,
                            length;
                        if (node.usedBytes === 0 && position === 0)
                            return node.contents = buffer.slice(offset, offset + length),
                            node.usedBytes = length,
                            length;
                        if (position + length <= node.usedBytes)
                            return node.contents.set(buffer.subarray(offset, offset + length), position),
                            length
                    }
                    if (MEMFS.expandFileStorage(node, position + length),
                    node.contents.subarray && buffer.subarray)
                        node.contents.set(buffer.subarray(offset, offset + length), position);
                    else
                        for (i = 0; i < length; i++)
                            node.contents[position + i] = buffer[offset + i];
                    return node.usedBytes = Math.max(node.usedBytes, position + length),
                    length
                },
                llseek(stream, offset, whence) {
                    var position = offset;
                    if (whence === 1 ? position += stream.position : whence === 2 && FS.isFile(stream.node.mode) && (position += stream.node.usedBytes),
                    position < 0)
                        throw new FS.ErrnoError(28);
                    return position
                },
                ...
                mmap(stream, length, position, prot, flags) {
                    ...
                    if (!(flags & 2) && contents.buffer === HEAP8.buffer)
                        ...
                        if ((position > 0 || position + length < contents.length) && (contents.subarray ? contents = contents.subarray(position, position + length) : contents = Array.prototype.slice.call(contents, position, position + length)),
                        allocated = !0,
                ...
                msync(stream, buffer, offset, length) {
                    return MEMFS.stream_ops.write(stream, buffer, 0, length, offset, !1),
                    0
                }
            }
        }
          ...
          , FS_handledByPreloadPlugin = (byteArray, fullname, finish, onerror) => {
            ...
            return preloadPlugins.forEach(plugin => {
                if (handled)
                    return;
                plugin.canHandle(fullname) && (plugin.handle(byteArray, fullname, finish, onerror),
                handled = !0)
            }
            ),
            handled
          ...
          , FS_createPreloadedFile = (parent, name, url, canRead, canWrite, onload, onerror, dontCreateFile, canOwn, preFinish) => {
            ...
            function processData(byteArray) {
                function finish(byteArray) {
                    preFinish?.(),
                    dontCreateFile || FS_createDataFile(parent, name, byteArray, canRead, canWrite, canOwn),
                    onload?.(),
            ...
            typeof url == "string" ? asyncLoad(url, processData, onerror) : processData(url)
        }
          ...
          , IDBFS = {
            ...
            indexedDB: () => {
                if (typeof indexedDB != "undefined")
                    return indexedDB;
                var ret = null;
                return typeof window == "object" && (ret = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB),
                ret
            }
            ,
            DB_VERSION: 21,
            ...
            queuePersist: mount => {
                function onPersistComplete() {
                    mount.idbPersistState === "again" ? startPersist() : mount.idbPersistState = 0
                }
                function startPersist() {
                    mount.idbPersistState = "idb",
                    IDBFS.syncfs(mount, !1, onPersistComplete)
                }
                mount.idbPersistState ? mount.idbPersistState === "idb" && (mount.idbPersistState = "again") : mount.idbPersistState = setTimeout(startPersist, 0)
            }
            ,
            mount: mount => {
                if (mnt = MEMFS.mount(mount),
                mount?.opts?.autoPersist) {
                    mnt.idbPersistState = 0;
                    var mnt, memfs_node_ops = mnt.node_ops;
                    mnt.node_ops = Object.assign({}, mnt.node_ops),
                    mnt.node_ops.mknod = (parent, name, mode, dev) => {
                        var node = memfs_node_ops.mknod(parent, name, mode, dev);
                        return node.node_ops = mnt.node_ops,
                        node.idbfs_mount = mnt.mount,
                        node.memfs_stream_ops = node.stream_ops,
                        node.stream_ops = Object.assign({}, node.stream_ops),
                        node.stream_ops.write = (stream, buffer, offset, length, position, canOwn) => (stream.node.isModified = !0,
                        node.memfs_stream_ops.write(stream, buffer, offset, length, position, canOwn)),
                        node.stream_ops.close = stream => {
                            var n = stream.node;
                            if (n.isModified && (IDBFS.queuePersist(n.idbfs_mount),
                            n.isModified = !1),
                            n.memfs_stream_ops.close)
            ...
            getDB: (name, callback) => {
                ...
                req.onupgradeneeded = e => {
                    var fileStore, db = e.target.result, transaction = e.target.transaction;
                    db.objectStoreNames.contains(IDBFS.DB_STORE_NAME) ? fileStore = transaction.objectStore(IDBFS.DB_STORE_NAME) : fileStore = db.createObjectStore(IDBFS.DB_STORE_NAME),
                    fileStore.indexNames.contains("timestamp") || fileStore.createIndex("timestamp", "timestamp", {
                        unique: !1
                    })
                }
                ,
            ...
            getLocalSet: (mount, callback) => {
                ...
                for (var check = FS.readdir(mount.mountpoint).filter(isRealDir).map(toAbsolute(mount.mountpoint)); check.length; ) {
                    ...
                    FS.isDir(stat.mode) && check.push(...FS.readdir(path).filter(isRealDir).map(toAbsolute(path))),
                    entries[path] = {
            ...
            loadLocalEntry: (path, callback) => {
                try {
                    var stat, node, lookup = FS.lookupPath(path);
                    node = lookup.node,
                    stat = FS.stat(path)
                } catch (e) {
                    return callback(e)
                }
                return FS.isDir(stat.mode) ? callback(null, {
                    timestamp: stat.mtime,
                    mode: stat.mode
                }) : FS.isFile(stat.mode) ? (node.contents = MEMFS.getFileDataAsTypedArray(node),
                callback(null, {
                    timestamp: stat.mtime,
                    mode: stat.mode,
            ...
            reconcile: (src, dst, callback) => {
                ...
                Object.keys(src.entries).forEach(function(key) {
                    var e = src.entries[key]
                      , e2 = dst.entries[key];
                    (!e2 || e.timestamp.getTime() != e2.timestamp.getTime()) && (create.push(key),
                    total++)
                });
                var remove = [];
                if (Object.keys(dst.entries).forEach(function(key) {
                    src.entries[key] || (remove.push(key),
                    total++)
                }),
                !total)
                    return callback(null);
                var errored = !1
                  , db = src.type === "remote" ? src.db : dst.db
                ...
                create.sort().forEach(path => {
                    dst.type === "local" ? IDBFS.loadRemoteEntry(store, path, (err, entry) => {
                        if (err)
                            return done(err);
                ...
                remove.sort().reverse().forEach(path => {
                    dst.type === "local" ? IDBFS.removeLocalEntry(path, done) : IDBFS.removeRemoteEntry(store, path, done)
                }
                )
            }
        }
          , FS = {
            ...
            getPath(node) {
                for (; !0; ) {
                    if (FS.isRoot(node)) {
                        var path, mount = node.mount.mountpoint;
            ...
            flagsToPermissionString(flag) {
                ...
                return flag & 512 && (perms += "w"),
                perms
            },
            nodePermissions(node, perms) {
                return FS.ignorePermissions ? 0 : perms.includes("r") && !(node.mode & 292) ? 2 : perms.includes("w") && !(node.mode & 146) ? 2 : perms.includes("x") && !(node.mode & 73) ? 2 : 0
            },
            mayLookup(dir) {
            ...
            mayDelete(dir, name, isdir) {
                ...
                var node, errCode = FS.nodePermissions(dir, "wx");
                if (errCode)
                    return errCode;
                if (isdir) {
                    {
            ...
            createStream(stream, fd=-1) {
                return stream = Object.assign(new FS.FSStream, stream),
                fd == -1 && (fd = FS.nextfd()),
                stream.fd = fd,
            ...
            dupStream(origStream, fd=-1) {
                ...
                return stream.stream_ops?.dup?.(stream),
                stream
            },
            chrdev_stream_ops: {
                open(stream) {
            ...
            getMounts(mount) {
                for (var m, mounts = [], check = [mount]; check.length; )
                    m = check.pop(),
                    mounts.push(m),
                    check.push(...m.mounts);
                return mounts
            ...
            syncfs(populate, callback) {
                typeof populate == "function" && (callback = populate,
                populate = !1),
                FS.syncFSRequests++,
            ...
            mount(type, opts, mountpoint) {
                ...
                return mountRoot.mount = mount,
                mount.root = mountRoot,
                root ? FS.root = mountRoot : node && (node.mounted = mount,
                node.mount && node.mount.mounts.push(mount)),
                mountRoot
            },
            unmount(mountpoint) {
                var idx, lookup = FS.lookupPath(mountpoint, {
                    follow_mount: !1
                });
                ...
                node.mounted = null,
                idx = node.mount.mounts.indexOf(mount),
            ...
            create(path, mode) {
                return mode = mode !== 0[0] ? mode : 438,
                mode &= 4095,
                mode |= 32768,
                FS.mknod(path, mode, 0)
            },
            mkdir(path, mode) {
                return mode = mode !== 0[0] ? mode : 511,
                mode &= 511 | 512,
                mode |= 16384,
                FS.mknod(path, mode, 0)
            },
            ...
            mkdev(path, mode, dev) {
                return typeof dev == "undefined" && (dev = mode,
                mode = 438),
                mode |= 8192,
            ...
            rename(old_path, new_path) {
                var new_node, old_dirname = PATH.dirname(old_path), new_dirname = PATH.dirname(new_path), old_name = PATH.basename(old_path), new_name = PATH.basename(new_path), lookup = FS.lookupPath(old_path, {
                    parent: !0
                ...
                if (relative.charAt(0) !== ".")
                    throw new FS.ErrnoError(28);
                if (relative = PATH_FS.relative(new_path, old_dirname),
                relative.charAt(0) !== ".")
                    throw new FS.ErrnoError(55);
                try {
                    new_node = FS.lookupNode(new_dir, new_name)
                ...
                if (errCode)
                    throw new FS.ErrnoError(errCode);
                if (errCode = new_node ? FS.mayDelete(new_dir, new_name, isdir) : FS.mayCreate(new_dir, new_name),
                errCode)
                    throw new FS.ErrnoError(errCode);
                ...
                if (FS.isMountpoint(old_node) || new_node && FS.isMountpoint(new_node))
                    throw new FS.ErrnoError(10);
                if (new_dir !== old_dir && (errCode = FS.nodePermissions(old_dir, "w"),
                errCode))
                    throw new FS.ErrnoError(errCode);
            ...
            chmod(path, mode, dontFollow) {
                if (typeof path == "string") {
                    var node, lookup = FS.lookupPath(path, {
                        follow: !dontFollow
                    });
                    node = lookup.node
                } else
            ...
            truncate(path, len) {
                ...
                if (!FS.isFile(node.mode))
                    throw new FS.ErrnoError(28);
                var node, errCode = FS.nodePermissions(node, "w");
                if (errCode)
                    throw new FS.ErrnoError(errCode);
                node.node_ops.setattr(node, {
                    size: len,
            ...
            open(path, flags, mode) {
                if (path === "")
                    throw new FS.ErrnoError(44);
                if (flags = typeof flags == "string" ? FS_modeStringToFlags(flags) : flags,
                flags & 64 ? (mode = typeof mode == "undefined" ? 438 : mode,
                mode = mode & 4095 | 32768) : mode = 0,
                typeof path == "object")
                    node = path;
                else {
                    path = PATH.normalize(path);
                ...
                flags & 65536 && !FS.isDir(node.mode))
                    throw new FS.ErrnoError(54);
                if (!created) {
                ...
                return stream.stream_ops.open && stream.stream_ops.open(stream),
                Module.logReadFiles && !(flags & 1) && (FS.readFiles || (FS.readFiles = {}),
                path in FS.readFiles || (FS.readFiles[path] = 1)),
                stream
            },
            close(stream) {
            ...
            llseek(stream, offset, whence) {
                ...
                if (whence != 0 && whence != 1 && whence != 2)
                    throw new FS.ErrnoError(28);
                return stream.position = stream.stream_ops.llseek(stream, offset, whence),
                stream.ungotten = [],
                stream.position
            },
            read(stream, buffer, offset, length, position) {
                ...
                return seeking || (stream.position += bytesRead),
                bytesRead
            },
            write(stream, buffer, offset, length, position, canOwn) {
                if (length < 0 || position < 0)
                ...
                return seeking || (stream.position += bytesWritten),
                bytesWritten
            },
            allocate(stream, offset, length) {
                if (FS.isClosed(stream))
            ...
            readFile(path, opts={}) {
                ...
                return FS.read(stream, buf, 0, length, 0),
                opts.encoding === "utf8" ? ret = UTF8ArrayToString(buf, 0) : opts.encoding === "binary" && (ret = buf),
                FS.close(stream),
                ret
            },
            writeFile(path, data, opts={}) {
                ...
                if (typeof data == "string") {
                    ...
                    FS.write(stream, buf, 0, actualNumBytes, 0[0], opts.canOwn)
                } else if (ArrayBuffer.isView(data))
                    FS.write(stream, data, 0, data.byteLength, 0[0], opts.canOwn);
                else
            ...
            createDefaultDevices() {
                ...
                var randomBuffer = new Uint8Array(1024)
                  , randomLeft = 0
                  , randomByte = () => (randomLeft === 0 && (randomLeft = randomFill(randomBuffer).byteLength),
            ...
            createStandardStreams() {
                ...
                var stdin = FS.open("/dev/stdin", 0)
                  , stdout = FS.open("/dev/stdout", 1)
                  , stderr = FS.open("/dev/stderr", 1)
            },
            staticInit() {
                [44].forEach(code => {
            ...
            analyzePath(path, dontResolveLastLink) {
                try {
                    var ret, lookup = FS.lookupPath(path, {
            ...
            createFile(parent, name, properties, canRead, canWrite) {
                var path = PATH.join2(typeof parent == "string" ? parent : FS.getPath(parent), name)
                  , mode = FS_getMode(canRead, canWrite);
                return FS.create(path, mode)
            },
            createDataFile(parent, name, data, canRead, canWrite, canOwn) {
                var path = name;
                parent && (parent = typeof parent == "string" ? parent : FS.getPath(parent),
            ...
            createDevice(parent, name, input, output) {
                var dev, path = PATH.join2(typeof parent == "string" ? parent : FS.getPath(parent), name), mode = FS_getMode(!!input, !!output);
                return FS.createDevice.major || (FS.createDevice.major = 64),
                dev = FS.makedev(FS.createDevice.major++, 0),
            ...
            createLazyFile(parent, name, url, canRead, canWrite) {
                class LazyUint8Array {
                    ...
                    cacheLength() {
                        ...
                        var xhr, header, datalength = +xhr.getResponseHeader("Content-length"), hasByteServing = (header = xhr.getResponseHeader("Accept-Ranges")) && header === "bytes", usesGzip = (header = xhr.getResponseHeader("Content-Encoding")) && header === "gzip", chunkSize = 1024 * 1024;
                        ...
                        var doXHR = (from, to) => {
                            ...
                            !(xhr.status >= 200 && xhr.status < 300 || xhr.status === 304))
                                throw new Error("Couldn't load " + url + ". Status: " + xhr.status);
                            return xhr.response !== 0[0] ? new Uint8Array(xhr.response || []) : intArrayFromString(xhr.responseText || "", !0)
                        }
                          , lazyArray = this;
                        ...
                        (usesGzip || !datalength) && (chunkSize = datalength = 1,
                ...
                properties.contents ? node.contents = properties.contents : properties.url && (node.contents = null,
                ...
                var stream_ops = {}
                  , keys = Object.keys(node.stream_ops);
                keys.forEach(key => {
                ...
                return stream_ops.read = (stream, buffer, offset, length, position) => (FS.forceLoadFile(node),
                writeChunks(stream, buffer, offset, length, position)),
                stream_ops.mmap = (stream, length, position) => {
                    FS.forceLoadFile(node);
                    var ptr = mmapAlloc(length);
                    if (!ptr)
                        throw new FS.ErrnoError(48);
                    return writeChunks(stream, HEAP8, ptr, length, position),
                    {
                        ptr,
                        allocated: !0
                    }
          ...
          , SYSCALLS = {
            ...
            calculateAt(dirfd, path, allowEmpty) {
                ...
                if (dirfd === -100)
                    ...
                    var dir, dirstream = SYSCALLS.getStreamFromFD(dirfd);
                    dir = dirstream.path
                }
                if (path.length == 0) {
                    if (!allowEmpty)
            ...
            doStat(func, path, buf) {
                ...
                return HEAP64[buf + 40 >> 3] = BigInt(Math.floor(atime / 1e3)),
        ...
        function ___syscall_chdir(path) {
            try {
                return path = SYSCALLS.getStr(path),
                FS.chdir(path),
                0
            } catch (e) {
        ...
        function ___syscall_faccessat(dirfd, path, amode) {
            try {
                if (path = SYSCALLS.getStr(path),
        ...
        function ___syscall_fcntl64(fd, cmd, varargs) {
            ...
            try {
                var arg, newStream, stream = SYSCALLS.getStreamFromFD(fd);
                switch (cmd) {
                case 0:
                    {
                        if (arg = syscallGetVarargI(),
                        arg < 0)
                            return -28;
                        for (; FS.streams[arg]; )
                            arg++;
                        return newStream = FS.dupStream(stream, arg),
                        newStream.fd
                    }
                case 1:
                case 2:
        ...
        function ___syscall_ftruncate64(fd, length) {
            ...
            try {
                return length != length ? 61 : (FS.ftruncate(fd, length),
                0)
            } catch (e) {
                if (typeof FS == "undefined" || e.name !== "ErrnoError")
                    throw e;
        ...
        function ___syscall_getcwd(buf, size) {
            try {
                ...
                return size < cwdLengthInBytes ? -68 : (stringToUTF8(cwd, buf, size),
                cwdLengthInBytes)
            } catch (e) {
                if (typeof FS == "undefined" || e.name !== "ErrnoError")
                    throw e;
        ...
        function ___syscall_getdents64(fd, dirp, count) {
            try {
                ...
                for (var id, type, struct_size = 280, pos = 0, off = FS.llseek(stream, 0, 1), idx = Math.floor(off / struct_size); idx < stream.getdents.length && pos + struct_size <= count; ) {
                ...
                return FS.llseek(stream, idx * struct_size, 0),
                pos
            } catch (e) {
                if (typeof FS == "undefined" || e.name !== "ErrnoError")
                    throw e;
        ...
        function ___syscall_lstat64(path, buf) {
            try {
                return path = SYSCALLS.getStr(path),
                SYSCALLS.doStat(FS.lstat, path, buf)
            } catch (e) {
                if (typeof FS == "undefined" || e.name !== "ErrnoError")
                    throw e;
                return -e.errno
        ...
        function ___syscall_mknodat(dirfd, path, mode, dev) {
            try {
                switch (path = SYSCALLS.getStr(path),
                path = SYSCALLS.calculateAt(dirfd, path),
                mode & 61440) {
                case 32768:
                case 8192:
        ...
        function ___syscall_newfstatat(dirfd, path, buf, flags) {
            try {
                ...
                var nofollow = flags & 256
                  , allowEmpty = flags & 4096;
                return flags = flags & ~6400,
        ...
        function ___syscall_readlinkat(dirfd, path, buf, bufsize) {
            try {
                ...
                return stringToUTF8(ret, buf, bufsize + 1),
        ...
        function ___syscall_renameat(olddirfd, oldpath, newdirfd, newpath) {
            try {
                return oldpath = SYSCALLS.getStr(oldpath),
        ...
        function ___syscall_symlink(target, linkpath) {
            try {
                return target = SYSCALLS.getStr(target),
        ...
        function ___syscall_unlinkat(dirfd, path, flags) {
            try {
                return path = SYSCALLS.getStr(path),
                path = SYSCALLS.calculateAt(dirfd, path),
                flags === 0 ? FS.unlink(path) : flags === 512 ? FS.rmdir(path) : abort("Invalid flags passed to unlinkat"),
                0
            } catch (e) {
                if (typeof FS == "undefined" || e.name !== "ErrnoError")
                    throw e;
        ...
        function __gmtime_js(time, tmPtr) {
            ...
            var start = Date.UTC(date.getUTCFullYear(), 0, 1, 0, 0, 0, 0)
              , yday = (date.getTime() - start) / (1e3 * 60 * 60 * 24) | 0;
        ...
        var isLeapYear = year => year % 4 === 0 && (year % 100 !== 0 || year % 400 === 0)
          ...
          , ydayFromDate = date => {
            var leap = isLeapYear(date.getFullYear())
              , monthDaysCumulative = leap ? MONTH_DAYS_LEAP_CUMULATIVE : MONTH_DAYS_REGULAR_CUMULATIVE
              , yday = monthDaysCumulative[date.getMonth()] + date.getDate() - 1;
            return yday
        }
        ;
        function __localtime_js(time, tmPtr) {
            ...
            var start = new Date(date.getFullYear(),0,1)
              ...
              , dst = (summerOffset != winterOffset && date.getTimezoneOffset() == Math.min(winterOffset, summerOffset)) | 0;
            HEAP32[tmPtr + 32 >> 2] = dst
        }
        var moduleRtn, readyPromiseResolve, readyPromiseReject, readAsync, readBinary, out, err, wasmBinary, wasmMemory, EXITSTATUS, HEAP8, HEAPU8, HEAP16, HEAPU16, HEAP32, HEAPU32, HEAPF32, HEAP64, HEAPU64, HEAPF64, wasmBinaryFile, wasmTable, _emscripten_get_now, GLctx, calledRun, __tzset_js = (timezone, daylight, std_name, dst_name) => {
            ...
            HEAP32[daylight >> 2] = +(winterOffset != summerOffset);
            ...
            summerOffset < winterOffset ? (stringToUTF8(winterName, std_name, 17),
            stringToUTF8(summerName, dst_name, 17)) : (stringToUTF8(winterName, dst_name, 17),
            stringToUTF8(summerName, std_name, 17))
        }
        , runtimeKeepaliveCounter = 0, runtimeKeepalivePush = () => {
            runtimeKeepaliveCounter += 1
        }
        , _emscripten_set_main_loop_timing = (mode, value) => {
            if (Browser.mainLoop.timingMode = mode,
                ...
                if (typeof Browser.setImmediate == "undefined")
                    if (typeof setImmediate == "undefined") {
                        var setImmediates = []
                          ...
                          , Browser_setImmediate_messageHandler = event => {
                            (event.data === emscriptenMainLoopMessageId || event.data.target === emscriptenMainLoopMessageId) && (event.stopPropagation(),
                            setImmediates.shift()())
                        }
                        ;
                        addEventListener("message", Browser_setImmediate_messageHandler, !0),
                        Browser.setImmediate = function(func) {
                            setImmediates.push(func),
                            ENVIRONMENT_IS_WORKER ? (Module.setImmediates ??= [],
        ...
        , _emscripten_get_now = () => performance.now(), webgl_enable_ANGLE_instanced_arrays = ctx => {
            var ext = ctx.getExtension("ANGLE_instanced_arrays");
            if (ext)
                return ctx.vertexAttribDivisor = (index, divisor) => ext.vertexAttribDivisorANGLE(index, divisor),
        ...
        , webgl_enable_OES_vertex_array_object = ctx => {
            ...
            if (ext)
                return ctx.createVertexArray = () => ext.createVertexArrayOES(),
        ...
        , webgl_enable_WEBGL_draw_buffers = ctx => {
            ...
            if (ext)
                return ctx.drawBuffers = (n, bufs) => ext.drawBuffersWEBGL(n, bufs),
                1
        }
        , webgl_enable_WEBGL_draw_instanced_base_vertex_base_instance = ctx => !!(ctx.dibvbi = ctx.getExtension("WEBGL_draw_instanced_base_vertex_base_instance")), webgl_enable_WEBGL_multi_draw_instanced_base_vertex_base_instance = ctx => !!(ctx.mdibvbi = ctx.getExtension("WEBGL_multi_draw_instanced_base_vertex_base_instance")), webgl_enable_WEBGL_multi_draw = ctx => !!(ctx.multiDrawWebgl = ctx.getExtension("WEBGL_multi_draw")), getEmscriptenSupportedExtensions = ctx => {
        ...
        , GL = {
            ...
            genObject: (n, buffers, createFunction, objectTable) => {
                for (i = 0; i < n; i++) {
                    var i, buffer = GLctx[createFunction](), id = buffer && GL.getNewId(objectTable);
                    buffer ? (buffer.name = id,
                    objectTable[id] = buffer) : GL.recordError(1282),
                    HEAP32[buffers + i * 4 >> 2] = id
                }
            }
            ,
            getSource: (shader, count, string, length) => {
                for (var len, source = "", i = 0; i < count; ++i)
                    len = length ? HEAPU32[length + i * 4 >> 2] : 0[0],
                    source += UTF8ToString(HEAPU32[string + i * 4 >> 2], len);
                return source
            }
            ,
            createContext: (canvas, webGLContextAttributes) => {
                if (webGLContextAttributes.renderViaOffscreenBackBuffer && (webGLContextAttributes.preserveDrawingBuffer = !0),
                ctx = webGLContextAttributes.majorVersion > 1 ? canvas.getContext("webgl2", webGLContextAttributes) : canvas.getContext("webgl", webGLContextAttributes),
                !ctx)
                    return 0;
                var ctx, handle = GL.registerContext(ctx, webGLContextAttributes);
                return handle
            }
            ,
            enableOffscreenFramebufferAttributes: webGLContextAttributes => {
                webGLContextAttributes.renderViaOffscreenBackBuffer = !0,
                webGLContextAttributes.preserveDrawingBuffer = !0
            }
            ,
            createOffscreenFramebuffer: context => {
                ...
                context.defaultFboForbidBlitFramebuffer = !1,
                gl.getContextAttributes().antialias && (context.defaultFboForbidBlitFramebuffer = !0),
                ...
                var gl, fbo, depthTarget = gl.createRenderbuffer();
                ...
                gl.useProgram(null),
                context.defaultVao = 0[0],
                gl.createVertexArray && (context.defaultVao = gl.createVertexArray(),
            ...
            resizeOffscreenFramebuffer: context => {
                ...
                if (context.defaultDepthTarget) {
                    var gl, prevRenderBufferBinding = gl.getParameter(36007);
                    gl.bindRenderbuffer(36161, context.defaultDepthTarget),
            ...
            blitOffscreenFramebuffer: context => {
                var gl = context.GLctx
                  , prevScissorTest = gl.getParameter(3089);
                prevScissorTest && gl.disable(3089);
                ...
                if (gl.blitFramebuffer && !context.defaultFboForbidBlitFramebuffer)
                    ...
                    prevBlend && gl.disable(3042);
                    var prevCullFace = gl.getParameter(2884);
                    prevCullFace && gl.disable(2884);
                    var prevDepthTest = gl.getParameter(2929);
                    prevDepthTest && gl.disable(2929);
                    var prevStencilTest = gl.getParameter(2960);
                    prevStencilTest && gl.disable(2960);
                    function draw() {
                        gl.vertexAttribPointer(context.blitPosLoc, 2, 5126, !1, 0, 0),
                        gl.drawArrays(5, 0, 4)
                    }
                    if (context.defaultVao) {
                        ...
                        for (var prevVertexAttribPointer = {
                        ...
                        }, maxVertexAttribs = gl.getParameter(34921), prevVertexAttribEnables = [], i = 0; i < maxVertexAttribs; ++i) {
                            var prevEnabled = gl.getVertexAttrib(i, 34338)
                              , wantEnabled = i == context.blitPosLoc;
                            prevEnabled && !wantEnabled && gl.disableVertexAttribArray(i),
                            !prevEnabled && wantEnabled && gl.enableVertexAttribArray(i),
                            prevVertexAttribEnables[i] = prevEnabled
                        }
                        draw();
                        for (i = 0; i < maxVertexAttribs; ++i) {
                            var prevEnabled = prevVertexAttribEnables[i]
                              , nowEnabled = i == context.blitPosLoc;
                            prevEnabled && !nowEnabled && gl.enableVertexAttribArray(i),
                            !prevEnabled && nowEnabled && gl.disableVertexAttribArray(i)
                        }
                    ...
                    prevStencilTest && gl.enable(2960),
                    prevDepthTest && gl.enable(2929),
                    prevCullFace && gl.enable(2884),
                    prevBlend && gl.enable(3042),
                    gl.bindTexture(3553, prevTextureBinding),
                ...
                prevScissorTest && gl.enable(3089)
            }
            ,
            registerContext: (ctx, webGLContextAttributes) => {
                ...
                return ctx.canvas && (ctx.canvas.GLctxObject = context),
                GL.contexts[handle] = context,
                (typeof webGLContextAttributes.enableExtensionsByDefault == "undefined" || webGLContextAttributes.enableExtensionsByDefault) && GL.initExtensions(context),
                webGLContextAttributes.renderViaOffscreenBackBuffer && GL.createOffscreenFramebuffer(context),
                handle
            }
            ,
            makeContextCurrent: contextHandle => (GL.currentContext = GL.contexts[contextHandle],
            Module.ctx = GLctx = GL.currentContext?.GLctx,
            !contextHandle || !!GLctx),
            getContext: contextHandle => GL.contexts[contextHandle],
            deleteContext: contextHandle => {
                GL.currentContext === GL.contexts[contextHandle] && (GL.currentContext = null),
                typeof JSEvents == "object" && JSEvents.removeAllHandlersOnTarget(GL.contexts[contextHandle].GLctx.canvas),
                GL.contexts[contextHandle] && GL.contexts[contextHandle].GLctx.canvas && (GL.contexts[contextHandle].GLctx.canvas.GLctxObject = 0[0]),
                GL.contexts[contextHandle] = null
            }
            ,
            initExtensions: context => {
                if (context ||= GL.currentContext,
                ...
                context.version >= 2 && (GLctx.disjointTimerQueryExt = GLctx.getExtension("EXT_disjoint_timer_query_webgl2")),
                (context.version < 2 || !GLctx.disjointTimerQueryExt) && (GLctx.disjointTimerQueryExt = GLctx.getExtension("EXT_disjoint_timer_query")),
                webgl_enable_WEBGL_multi_draw(GLctx),
                getEmscriptenSupportedExtensions(GLctx).forEach(ext => {
                    !ext.includes("lose_context") && !ext.includes("debug") && GLctx.getExtension(ext)
                }
                )
            }
        }, _emscripten_webgl_do_commit_frame = () => !GL.currentContext || !GL.currentContext.GLctx ? -3 : GL.currentContext.defaultFbo ? (GL.blitOffscreenFramebuffer(GL.currentContext),
        0) : GL.currentContext.attributes.explicitSwapControl ? 0 : -3, _emscripten_webgl_commit_frame = _emscripten_webgl_do_commit_frame, keepRuntimeAlive = () => noExitRuntime || runtimeKeepaliveCounter > 0, _proc_exit = code => {
            EXITSTATUS = code,
            keepRuntimeAlive() || (Module.onExit?.(code),
            ABORT = !0),
        ...
        , _exit = exitJS, handleException = e => {
        ...
        , setMainLoop = (browserIterationFunc, fps, simulateInfiniteLoop, arg, noSetTiming) => {
            ...
            function checkIsRunning() {
                return !(thisMainLoopId < Browser.mainLoop.currentlyRunningMainloop) || (runtimeKeepalivePop(),
                maybeExit(),
                !1)
            }
            ...
            Browser.mainLoop.runner = function() {
                ...
                if (Browser.mainLoop.queue.length > 0) {
                    var start = Date.now()
                      , blocker = Browser.mainLoop.queue.shift();
                    if (blocker.func(blocker.arg),
                    Browser.mainLoop.remainingBlockers) {
                        var remaining = Browser.mainLoop.remainingBlockers
                          , next = remaining % 1 == 0 ? remaining - 1 : Math.floor(remaining);
                        blocker.counted ? Browser.mainLoop.remainingBlockers = next : (next = next + .5,
                ...
                Browser.mainLoop.timingMode == 1 && Browser.mainLoop.timingValue > 1 && Browser.mainLoop.currentFrameNumber % Browser.mainLoop.timingValue != 0) {
                    Browser.mainLoop.scheduler();
                    return
                }
                if (Browser.mainLoop.timingMode == 0 && (Browser.mainLoop.tickStartTime = _emscripten_get_now()),
                Browser.mainLoop.runIter(browserIterationFunc),
                !checkIsRunning())
                    return;
                typeof SDL == "object" && SDL.audio?.queueNewAudioData?.(),
                Browser.mainLoop.scheduler()
            }
            ,
            noSetTiming || (fps && fps > 0 ? _emscripten_set_main_loop_timing(0, 1e3 / fps) : _emscripten_set_main_loop_timing(1, 1),
            Browser.mainLoop.scheduler()),
        ...
        , safeSetTimeout = (func, timeout) => (runtimeKeepalivePush(),
        setTimeout( () => {
            runtimeKeepalivePop(),
            callUserCallback(func)
        }
        , timeout)), warnOnce = text => {
            warnOnce.shown ||= {},
            warnOnce.shown[text] || (warnOnce.shown[text] = 1,
            err(text))
        }
        , Browser = {
            mainLoop: {
            ...
            init() {
                ...
                imagePlugin.handle = function(byteArray, name, onload, onerror) {
                    ...
                    b.size !== byteArray.length && (b = new Blob([new Uint8Array(byteArray).buffer],{
                        type: Browser.getMimetype(name)
                    }));
                    var b, url = URL.createObjectURL(b), img = new Image;
                    img.onload = () => {
                ...
                ,
                audioPlugin.handle = function(byteArray, name, onload) {
                    var done = !1;
                    ...
                    audio.onerror = function() {
                        ...
                        function encode64(data) {
                            for (var BASE = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", PAD = "=", ret = "", leftchar = 0, leftbits = 0, i = 0; i < data.length; i++)
                                for (leftchar = leftchar << 8 | data[i],
                                leftbits += 8; leftbits >= 6; ) {
                                    var curr = leftchar >> leftbits - 6 & 63;
                                    leftbits -= 6,
                                    ret += BASE[curr]
                                }
                            return leftbits == 2 ? (ret += BASE[(leftchar & 3) << 4],
                            ret += PAD + PAD) : leftbits == 4 && (ret += BASE[(leftchar & 15) << 2],
                            ret += PAD),
                            ret
                        }
                ...
                canvas && (canvas.requestPointerLock = canvas.requestPointerLock || canvas.mozRequestPointerLock || canvas.webkitRequestPointerLock || canvas.msRequestPointerLock || ( () => {}
                ),
                ...
                Module.elementPointerLock && canvas.addEventListener("click", ev => {
            ...
            createContext(canvas, useWebGL, setInModule, webGLContextAttributes) {
                if (useWebGL && Module.ctx && canvas == Module.canvas)
                    return Module.ctx;
                if (useWebGL) {
                    var ctx, contextHandle, attribute, contextAttributes = {
                        antialias: !1,
                        alpha: !1,
                        majorVersion: typeof WebGL2RenderingContext != "undefined" ? 2 : 1
                    };
                    if (webGLContextAttributes)
                        for (attribute in webGLContextAttributes)
                            contextAttributes[attribute] = webGLContextAttributes[attribute];
                    typeof GL != "undefined" && (contextHandle = GL.createContext(canvas, contextAttributes),
                    contextHandle && (ctx = GL.getContext(contextHandle).GLctx))
                } else
                    ctx = canvas.getContext("2d");
                return ctx ? (setInModule && (Module.ctx = ctx,
            ...
            requestFullscreen(lockPointer, resizeCanvas) {
                ...
                typeof Browser.lockPointer == "undefined" && (Browser.lockPointer = !0),
                typeof Browser.resizeCanvas == "undefined" && (Browser.resizeCanvas = !1);
                var canvas = Module.canvas;
                function fullscreenChange() {
                    Browser.isFullscreen = !1;
                    var canvasContainer = canvas.parentNode;
                    (document.fullscreenElement || document.mozFullScreenElement || document.msFullscreenElement || document.webkitFullscreenElement || document.webkitCurrentFullScreenElement) === canvasContainer ? (canvas.exitFullscreen = Browser.exitFullscreen,
                    Browser.lockPointer && canvas.requestPointerLock(),
                    Browser.isFullscreen = !0,
                    Browser.resizeCanvas ? Browser.setFullscreenCanvasSize() : Browser.updateCanvasDimensions(canvas)) : (canvasContainer.parentNode.insertBefore(canvas, canvasContainer),
                    canvasContainer.parentNode.removeChild(canvasContainer),
                    Browser.resizeCanvas ? Browser.setWindowedCanvasSize() : Browser.updateCanvasDimensions(canvas)),
                ...
                document.addEventListener("webkitfullscreenchange", fullscreenChange, !1),
            ...
            fakeRequestAnimationFrame(func) {
                ...
                Browser.nextRAF === 0)
                    ...
                    for (; now + 2 >= Browser.nextRAF; )
                        Browser.nextRAF += 1e3 / 60;
                var now, delay = Math.max(Browser.nextRAF - now, 0);
            ...
            calculateMouseCoords(pageX, pageY) {
                var rect = Module.canvas.getBoundingClientRect()
                  ...
                  , scrollX = typeof window.scrollX != "undefined" ? window.scrollX : window.pageXOffset
                  , scrollY = typeof window.scrollY != "undefined" ? window.scrollY : window.pageYOffset
                  , adjustedX = pageX - (scrollX + rect.left)
                  , adjustedY = pageY - (scrollY + rect.top)
            ...
            calculateMouseEvent(event) {
                if (Browser.pointerLock)
                    event.type != "mousemove" && "mozMovementX"in event ? Browser.mouseMovementX = Browser.mouseMovementY = 0 : (Browser.mouseMovementX = Browser.getMovementX(event),
            ...
            updateCanvasDimensions(canvas, wNative, hNative) {
                if (wNative && hNative ? (canvas.widthNative = wNative,
                canvas.heightNative = hNative) : (wNative = canvas.widthNative,
                ...
                (document.fullscreenElement || document.mozFullScreenElement || document.msFullscreenElement || document.webkitFullscreenElement || document.webkitCurrentFullScreenElement) === canvas.parentNode && typeof screen != "undefined") {
                    var h, w, factor = Math.min(screen.width / w, screen.height / h);
                    w = Math.round(w * factor),
                    h = Math.round(h * factor)
                }
                Browser.resizeCanvas ? (canvas.width != w && (canvas.width = w),
                canvas.height != h && (canvas.height = h),
                typeof canvas.style != "undefined" && (canvas.style.removeProperty("width"),
                canvas.style.removeProperty("height"))) : (canvas.width != wNative && (canvas.width = wNative),
                canvas.height != hNative && (canvas.height = hNative),
                typeof canvas.style != "undefined" && (w != wNative || h != hNative ? (canvas.style.setProperty("width", w + "px", "important"),
                canvas.style.setProperty("height", h + "px", "important")) : (canvas.style.removeProperty("width"),
        ...
        , _emscripten_date_now = () => Date.now(), _emscripten_force_exit = status => {
            __emscripten_runtime_keepalive_clear(),
            _exit(status)
        }
        , getHeapMax = () => 2147483648, _emscripten_get_heap_max = () => getHeapMax(), growMemory = size => {
            var b = wasmMemory.buffer
              , pages = (size - b.byteLength + 65535) / 65536;
            try {
                return wasmMemory.grow(pages),
                updateMemoryViews(),
                1
            } catch {}
        }
        , _emscripten_resize_heap = requestedSize => {
            ...
            if (requestedSize > maxHeapSize)
                return !1;
            for (var alignUp = (x, multiple) => x + (multiple - x % multiple) % multiple, cutDown = 1; cutDown <= 4; cutDown *= 2) {
        ...
        , JSEvents = {
            ...
            runDeferredCalls() {
                if (!JSEvents.canPerformEventHandlerRequests())
                    return;
                var call, deferredCalls = JSEvents.deferredCalls;
                JSEvents.deferredCalls = [];
                for (call of deferredCalls)
                    call.targetFunction(...call.argsList)
            },
            eventHandlers: [],
            removeAllHandlersOnTarget: (target, eventTypeString) => {
                for (var i = 0; i < JSEvents.eventHandlers.length; ++i)
                    JSEvents.eventHandlers[i].target == target && (!eventTypeString || eventTypeString == JSEvents.eventHandlers[i].eventTypeString) && JSEvents._removeHandler(i--)
            }
            ,
            _removeHandler(i) {
            ...
            registerOrRemoveHandler(eventHandler) {
                ...
                if (eventHandler.callbackfunc)
                    ...
                    for (var i = 0; i < JSEvents.eventHandlers.length; ++i)
                        JSEvents.eventHandlers[i].target == eventHandler.target && JSEvents.eventHandlers[i].eventTypeString == eventHandler.eventTypeString && JSEvents._removeHandler(i--);
                return 0
            },
            getNodeNameForTarget(target) {
                return target ? target == window ? "#window" : target == screen ? "#screen" : target?.nodeName || "" : ""
            },
            fullscreenEnabled() {
                return document.fullscreenEnabled || document.webkitFullscreenEnabled
            }
        }, maybeCStringToJsString = cString => cString > 2 ? UTF8ToString(cString) : cString, specialHTMLTargets = [0, typeof document != "undefined" ? document : 0, typeof window != "undefined" ? window : 0], findEventTarget = target => {
            target = maybeCStringToJsString(target);
            var domElement = specialHTMLTargets[target] || (typeof document != "undefined" ? document.querySelector(target) : 0[0]);
            return domElement
        }
        , findCanvasEventTarget = findEventTarget, _emscripten_set_canvas_element_size = (target, width, height) => {
            var canvas = findCanvasEventTarget(target);
            return canvas ? (canvas.width = width,
            canvas.height = height,
            canvas.GLctxObject && GL.resizeOffscreenFramebuffer(canvas.GLctxObject),
            0) : -4
        }
        ...
        , webglPowerPreferences = ["default", "low-power", "high-performance"], _emscripten_webgl_do_create_context = (target, attributes) => {
            var attr32 = attributes >> 2
              , powerPreference = HEAP32[attr32 + (8 >> 2)]
              , contextAttributes = {
              ...
              , canvas = findCanvasEventTarget(target);
            if (!canvas)
                return 0;
            contextAttributes.explicitSwapControl && !contextAttributes.renderViaOffscreenBackBuffer && (contextAttributes.renderViaOffscreenBackBuffer = !0);
            var contextHandle = GL.createContext(canvas, contextAttributes);
            return contextHandle
        }
        , _emscripten_webgl_create_context = _emscripten_webgl_do_create_context, _emscripten_webgl_destroy_context = contextHandle => {
            GL.currentContext == contextHandle && (GL.currentContext = 0),
            GL.deleteContext(contextHandle)
        }
        , _emscripten_webgl_enable_extension = (contextHandle, extension) => {
            var ext, context = GL.getContext(contextHandle), extString = UTF8ToString(extension);
            return extString.startsWith("GL_") && (extString = extString.substr(3)),
            extString == "ANGLE_instanced_arrays" && webgl_enable_ANGLE_instanced_arrays(GLctx),
            extString == "OES_vertex_array_object" && webgl_enable_OES_vertex_array_object(GLctx),
            extString == "WEBGL_draw_buffers" && webgl_enable_WEBGL_draw_buffers(GLctx),
            extString == "WEBGL_draw_instanced_base_vertex_base_instance" && webgl_enable_WEBGL_draw_instanced_base_vertex_base_instance(GLctx),
            extString == "WEBGL_multi_draw_instanced_base_vertex_base_instance" && webgl_enable_WEBGL_multi_draw_instanced_base_vertex_base_instance(GLctx),
            extString == "WEBGL_multi_draw" && webgl_enable_WEBGL_multi_draw(GLctx),
            ext = context.GLctx.getExtension(extString),
            !!ext
        }
        , stringToNewUTF8 = str => {
            var size = lengthBytesUTF8(str) + 1
              , ret = _malloc(size);
            return ret && stringToUTF8(str, ret, size),
            ret
        }
        , _emscripten_webgl_get_supported_extensions = () => stringToNewUTF8(GLctx.getSupportedExtensions().join(" ")), _emscripten_webgl_make_context_current = contextHandle => {
        ...
        , ENV = {}, getExecutableName = () => thisProgram || "./this.program", getEnvStrings = () => {
            if (!getEnvStrings.strings) {
                ...
                for (x in ENV)
                    ENV[x] === 0[0] ? delete env[x] : env[x] = ENV[x];
                var x, strings = [];
                for (x in env)
        ...
        , _environ_sizes_get = (penviron_count, penviron_buf_size) => {
            ...
            var bufSize = 0;
            return strings.forEach(string => bufSize += string.length + 1),
        ...
        var doReadv = (stream, iov, iovcnt, offset) => {
            ...
            i = 0; i < iovcnt; i++) {
                ...
                len = HEAPU32[iov + 4 >> 2],
                iov += 8;
                var ret, i, ptr, len, curr = FS.read(stream, HEAP8, ptr, len, offset);
                if (curr < 0)
                    return -1;
                if (ret += curr,
                curr < len)
                    break;
                typeof offset != "undefined" && (offset += curr)
            }
            return ret
        }
        ;
        ...
        function _fd_seek(fd, offset, whence, newOffset) {
            ...
            try {
                ...
                stream.getdents && offset === 0 && whence === 0 && (stream.getdents = null),
                0
            } catch (e) {
                if (typeof FS == "undefined" || e.name !== "ErrnoError")
                    throw e;
        ...
        var doWritev = (stream, iov, iovcnt, offset) => {
            ...
            i = 0; i < iovcnt; i++) {
                ...
                len = HEAPU32[iov + 4 >> 2],
                iov += 8;
                var ret, i, ptr, len, curr = FS.write(stream, HEAP8, ptr, len, offset);
                if (curr < 0)
                    return -1;
                ret += curr,
                typeof offset != "undefined" && (offset += curr)
        ...
        function _fd_write(fd, iov, iovcnt, pnum) {
            try {
                var stream = SYSCALLS.getStreamFromFD(fd)
                  , num = doWritev(stream, iov, iovcnt);
                return HEAPU32[pnum >> 2] = num,
                0
            } catch (e) {
                if (typeof FS == "undefined" || e.name !== "ErrnoError")
        ...
        var _glActiveTexture = x0 => GLctx.activeTexture(x0)
          ...
          , _glBeginTransformFeedback = x0 => GLctx.beginTransformFeedback(x0)
          , _glBindBuffer = (target, buffer) => {
            target == 35051 ? GLctx.currentPixelPackBufferBinding = buffer : target == 35052 && (GLctx.currentPixelUnpackBufferBinding = buffer),
            GLctx.bindBuffer(target, GL.buffers[buffer])
        }
          , _glBindBufferBase = (target, index, buffer) => {
            GLctx.bindBufferBase(target, index, GL.buffers[buffer])
        }
          , _glBindBufferRange = (target, index, buffer, offset, ptrsize) => {
          ...
          , _glBlendColor = (x0, x1, x2, x3) => GLctx.blendColor(x0, x1, x2, x3)
          , _glBlendEquation = x0 => GLctx.blendEquation(x0)
          , _glBlendFunc = (x0, x1) => GLctx.blendFunc(x0, x1)
          , _glBlendFuncSeparate = (x0, x1, x2, x3) => GLctx.blendFuncSeparate(x0, x1, x2, x3)
          , _glBlitFramebuffer = (x0, x1, x2, x3, x4, x5, x6, x7, x8, x9) => GLctx.blitFramebuffer(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9)
          , _glBufferData = (target, size, data, usage) => {
            if (GL.currentContext.version >= 2) {
                data && size ? GLctx.bufferData(target, HEAPU8, usage, data, size) : GLctx.bufferData(target, size, usage);
                return
            }
          ...
          , _glCheckFramebufferStatus = x0 => GLctx.checkFramebufferStatus(x0)
          , _glClear = x0 => GLctx.clear(x0)
          , _glClearBufferfv = (buffer, drawbuffer, value) => {
            GLctx.clearBufferfv(buffer, drawbuffer, HEAPF32, value >> 2)
        }
          , _glClearColor = (x0, x1, x2, x3) => GLctx.clearColor(x0, x1, x2, x3)
          , _glClearDepthf = x0 => GLctx.clearDepth(x0)
          ...
          , _glCompressedTexImage3D = (target, level, internalFormat, width, height, depth, border, imageSize, data) => {
            GLctx.currentPixelUnpackBufferBinding ? GLctx.compressedTexImage3D(target, level, internalFormat, width, height, depth, border, imageSize, data) : GLctx.compressedTexImage3D(target, level, internalFormat, width, height, depth, border, HEAPU8, data, imageSize)
        }
          , _glCompressedTexSubImage3D = (target, level, xoffset, yoffset, zoffset, width, height, depth, format, imageSize, data) => {
            GLctx.currentPixelUnpackBufferBinding ? GLctx.compressedTexSubImage3D(target, level, xoffset, yoffset, zoffset, width, height, depth, format, imageSize, data) : GLctx.compressedTexSubImage3D(target, level, xoffset, yoffset, zoffset, width, height, depth, format, HEAPU8, data, imageSize)
        }
          , _glCopyBufferSubData = (x0, x1, x2, x3, x4) => GLctx.copyBufferSubData(x0, x1, x2, x3, x4)
          , _glCreateProgram = () => {
          ...
          , _glCreateShader = shaderType => {
            ...
            return GL.shaders[id] = GLctx.createShader(shaderType),
            id
        }
          , _glCullFace = x0 => GLctx.cullFace(x0)
          , _glDeleteBuffers = (n, buffers) => {
            for (i = 0; i < n; i++) {
                var i, id = HEAP32[buffers + i * 4 >> 2], buffer = GL.buffers[id];
                if (!buffer)
                    continue;
                GLctx.deleteBuffer(buffer),
                buffer.name = 0,
                GL.buffers[id] = null,
                id == GLctx.currentPixelPackBufferBinding && (GLctx.currentPixelPackBufferBinding = 0),
                id == GLctx.currentPixelUnpackBufferBinding && (GLctx.currentPixelUnpackBufferBinding = 0)
            }
        }
          , _glDeleteFramebuffers = (n, framebuffers) => {
            for (i = 0; i < n; ++i) {
                var i, id = HEAP32[framebuffers + i * 4 >> 2], framebuffer = GL.framebuffers[id];
                if (!framebuffer)
                    continue;
          ...
          , _glDeleteQueries = (n, ids) => {
            for (i = 0; i < n; i++) {
                var i, id = HEAP32[ids + i * 4 >> 2], query = GL.queries[id];
                if (!query)
                    continue;
          ...
          , _glDeleteRenderbuffers = (n, renderbuffers) => {
            for (i = 0; i < n; i++) {
                var i, id = HEAP32[renderbuffers + i * 4 >> 2], renderbuffer = GL.renderbuffers[id];
                if (!renderbuffer)
                    continue;
          ...
          , _glDeleteTextures = (n, textures) => {
            for (i = 0; i < n; i++) {
                var i, id = HEAP32[textures + i * 4 >> 2], texture = GL.textures[id];
                if (!texture)
                    continue;
          ...
          , _glDeleteVertexArrays = (n, vaos) => {
            for (i = 0; i < n; i++) {
                var i, id = HEAP32[vaos + i * 4 >> 2];
                GLctx.deleteVertexArray(GL.vaos[id]),
                GL.vaos[id] = null
            }
        }
          , _glDepthFunc = x0 => GLctx.depthFunc(x0)
          , _glDepthMask = flag => {
          ...
          , _glFenceSync = (condition, flags) => {
            var id, sync = GLctx.fenceSync(condition, flags);
            return sync ? (id = GL.getNewId(GL.syncs),
            sync.name = id,
            GL.syncs[id] = sync,
            id) : 0
        }
          ...
          , _glFrontFace = x0 => GLctx.frontFace(x0)
          ...
          , _glGenerateMipmap = x0 => GLctx.generateMipmap(x0)
          ...
          , emscriptenWebGLGet = (name_, p, type) => {
            ...
            case 36346:
                ...
                break;
            case 36344:
                type != 0 && type != 1 && GL.recordError(1280);
                return;
            case 34814:
            case 36345:
                ret = 0;
                break;
            case 34466:
                var ret, i, formats = GLctx.getParameter(34467);
          ...
          , _glGetFloatv = (name_, p) => emscriptenWebGLGet(name_, p, 2)
          , _glGetInteger64v = (name_, p) => {
            emscriptenWebGLGet(name_, p, 1)
        }
          , _glGetIntegerv = (name_, p) => emscriptenWebGLGet(name_, p, 0)
          , _glGetProgramInfoLog = (program, maxLength, length, infoLog) => {
            log = GLctx.getProgramInfoLog(GL.programs[program]),
            log === null && (log = "(unknown error)");
            var log, numBytesWrittenExclNull = maxLength > 0 && infoLog ? stringToUTF8(log, infoLog, maxLength) : 0;
            length && (HEAP32[length >> 2] = numBytesWrittenExclNull)
        }
          , _glGetProgramiv = (program, pname, p) => {
            if (!p) {
                GL.recordError(1281);
                return
            }
            ...
            pname == 35716) {
                var i, log = GLctx.getProgramInfoLog(program);
                log === null && (log = "(unknown error)"),
          ...
          , _glGetShaderInfoLog = (shader, maxLength, length, infoLog) => {
            log = GLctx.getShaderInfoLog(GL.shaders[shader]),
            log === null && (log = "(unknown error)");
            var log, numBytesWrittenExclNull = maxLength > 0 && infoLog ? stringToUTF8(log, infoLog, maxLength) : 0;
            length && (HEAP32[length >> 2] = numBytesWrittenExclNull)
        }
          , _glGetShaderiv = (shader, pname, p) => {
            if (!p) {
                GL.recordError(1281);
                return
            }
            if (pname == 35716) {
                ...
                var log, source = GLctx.getShaderSource(GL.shaders[shader]), sourceLength = source ? source.length + 1 : 0;
          ...
          , _glGetString = name_ => {
            ...
            !ret) {
                ...
                case 37445:
                case 37446:
                    s = GLctx.getParameter(name_),
                    s || GL.recordError(1280),
                    ret = s ? stringToNewUTF8(s) : 0;
                    break;
                case 7938:
                    var glVersion = GLctx.getParameter(7938);
                    GL.currentContext.version >= 2 ? glVersion = `OpenGL ES 3.0 (${glVersion})` : glVersion = `OpenGL ES 2.0 (${glVersion})`,
                    ret = stringToNewUTF8(glVersion);
                    break;
                case 35724:
                    var ret, s, glslVersion = GLctx.getParameter(35724), ver_re = /^WebGL GLSL ES ([0-9]\.[0-9][0-9]?)(?:$| .*)/, ver_num = glslVersion.match(ver_re);
                    ver_num !== null && (ver_num[1].length == 3 && (ver_num[1] = ver_num[1] + "0"),
          ...
          , _glGetSynciv = (sync, pname, bufSize, length, values) => {
            ...
            ret !== null && (HEAP32[values >> 2] = ret,
            length && (HEAP32[length >> 2] = 1))
        }
          , _glGetUniformBlockIndex = (program, uniformBlockName) => GLctx.getUniformBlockIndex(GL.programs[program], UTF8ToString(uniformBlockName))
          , jstoi_q = str => parseInt(str)
          , webglGetLeftBracePos = name => name.slice(-1) == "]" && name.lastIndexOf("[")
          , webglPrepareUniformLocationsBeforeFirstUse = program => {
            var i, u, j, nm, sz, lb, id, uniformLocsById = program.uniformLocsById, uniformSizeAndIdsByName = program.uniformSizeAndIdsByName;
            if (!uniformLocsById) {
                ...
                for (i = 0; i < GLctx.getProgramParameter(program, 35718); ++i) {
                    var u = GLctx.getActiveUniform(program, i)
                      , nm = u.name
                      , sz = u.size
                      , lb = webglGetLeftBracePos(nm)
                      , arrayName = lb > 0 ? nm.slice(0, lb) : nm
            ...
            program = GL.programs[program]) {
                {
                    ...
                    leftBrace > 0 && (arrayIndex = jstoi_q(name.slice(leftBrace + 1)) >>> 0,
          ...
          , _glPixelStorei = (pname, param) => {
            pname == 3317 ? GL.unpackAlignment = param : pname == 3314 && (GL.unpackRowLength = param),
            GLctx.pixelStorei(pname, param)
        }
          , _glReadBuffer = x0 => GLctx.readBuffer(x0)
          , computeUnpackAlignedImageSize = (width, height, sizePerPixel) => {
            function roundedToNextMultipleOf(x, y) {
                return x + y - 1 & -y
            }
            var plainRowSize = (GL.unpackRowLength || width) * sizePerPixel
              , alignedRowSize = roundedToNextMultipleOf(plainRowSize, GL.unpackAlignment);
        ...
        type == 0 ? HEAP8 : type == 1 ? HEAPU8 : type == 2 ? HEAP16 : type == 4 ? HEAP32 : type == 6 ? HEAPF32 : type == 5 || type == 28922 || type == 28520 || type == 30779 || type == 30782 ? HEAPU32 : HEAPU16)
          , toTypedArrayIndex = (pointer, heap) => pointer >>> 31 - Math.clz32(heap.BYTES_PER_ELEMENT)
          , emscriptenWebGLGetTexPixelData = (type, format, width, height, pixels) => {
            var heap = heapObjectForWebGLType(type)
              , sizePerPixel = colorChannelsInGlTextureFormat(format) * heap.BYTES_PER_ELEMENT
              , bytes = computeUnpackAlignedImageSize(width, height, sizePerPixel);
          ...
          , _glReadPixels = (x, y, width, height, format, type, pixels) => {
            if (GL.currentContext.version >= 2) {
                ...
                var heap = heapObjectForWebGLType(type)
                  , target = toTypedArrayIndex(pixels, heap);
          ...
          , _glRenderbufferStorage = (x0, x1, x2, x3) => GLctx.renderbufferStorage(x0, x1, x2, x3)
          , _glRenderbufferStorageMultisample = (x0, x1, x2, x3, x4) => GLctx.renderbufferStorageMultisample(x0, x1, x2, x3, x4)
          , _glScissor = (x0, x1, x2, x3) => GLctx.scissor(x0, x1, x2, x3)
          ...
          , _glTexImage2D = (target, level, internalFormat, width, height, border, format, type, pixels) => {
            if (GL.currentContext.version >= 2) {
                ...
                if (pixels) {
                    var heap = heapObjectForWebGLType(type)
                      , index = toTypedArrayIndex(pixels, heap);
          ...
          , _glTexParameterf = (x0, x1, x2) => GLctx.texParameterf(x0, x1, x2)
          , _glTexParameteri = (x0, x1, x2) => GLctx.texParameteri(x0, x1, x2)
          , _glTexStorage2D = (x0, x1, x2, x3, x4) => GLctx.texStorage2D(x0, x1, x2, x3, x4)
          ...
          , _glTransformFeedbackVaryings = (program, count, varyings, bufferMode) => {
            program = GL.programs[program];
            for (var vars = [], i = 0; i < count; i++)
                vars.push(UTF8ToString(HEAP32[varyings + i * 4 >> 2]));
            GLctx.transformFeedbackVaryings(program, vars, bufferMode)
        }
          , webglGetUniformLocation = location => {
            if (p = GLctx.currentProgram,
            p) {
                var p, webglLoc = p.uniformLocsById[location];
                return typeof webglLoc == "number" && (p.uniformLocsById[location] = webglLoc = GLctx.getUniformLocation(p, p.uniformArrayNamesById[location] + (webglLoc > 0 ? `[${webglLoc}]` : ""))),
                webglLoc
            }
            GL.recordError(1282)
        }
          ...
          , _glUniform1iv = (location, count, value) => {
            ...
            if (count <= 288)
                for (var view = miniTempWebGLIntBuffers[count], i = 0; i < count; ++i)
          ...
          , _glUniform2fv = (location, count, value) => {
            ...
            if (count <= 144)
                for (var view = miniTempWebGLFloatBuffers[2 * count], i = 0; i < 2 * count; i += 2)
          ...
          , _glUniform2iv = (location, count, value) => {
            ...
            if (count <= 144)
                for (var view = miniTempWebGLIntBuffers[2 * count], i = 0; i < 2 * count; i += 2)
                    view[i] = HEAP32[value + 4 * i >> 2],
          ...
          , _glUniform3fv = (location, count, value) => {
            ...
            if (count <= 96)
                for (var view = miniTempWebGLFloatBuffers[3 * count], i = 0; i < 3 * count; i += 3)
                    view[i] = HEAPF32[value + 4 * i >> 2],
                    view[i + 1] = HEAPF32[value + (4 * i + 4) >> 2],
          ...
          , _glUniform4fv = (location, count, value) => {
            ...
            if (count <= 72) {
                var i, dst, view = miniTempWebGLFloatBuffers[4 * count], heap = HEAPF32;
                value = value >> 2;
          ...
          , _glUniformMatrix3fv = (location, count, transpose, value) => {
            ...
            if (count <= 32)
                for (var view = miniTempWebGLFloatBuffers[9 * count], i = 0; i < 9 * count; i += 9)
                    view[i] = HEAPF32[value + 4 * i >> 2],
                    view[i + 1] = HEAPF32[value + (4 * i + 4) >> 2],
                    view[i + 2] = HEAPF32[value + (4 * i + 8) >> 2],
          ...
          , _glUniformMatrix4fv = (location, count, transpose, value) => {
            ...
            if (count <= 18) {
                var i, dst, view = miniTempWebGLFloatBuffers[16 * count], heap = HEAPF32;
                value = value >> 2;
          ...
          , _glVertexAttrib4f = (x0, x1, x2, x3, x4) => GLctx.vertexAttrib4f(x0, x1, x2, x3, x4)
          , _glVertexAttribDivisor = (index, divisor) => {
            GLctx.vertexAttribDivisor(index, divisor)
        }
          , _glVertexAttribI4ui = (x0, x1, x2, x3, x4) => GLctx.vertexAttribI4ui(x0, x1, x2, x3, x4)
          ...
          , _glViewport = (x0, x1, x2, x3) => GLctx.viewport(x0, x1, x2, x3)
          , GodotRuntime = {
            ...
            parseStringArray: function(p_ptr, p_size) {
                ...
                return ptrs.forEach(function(ptr) {
            ...
            allocString: function(p_str) {
                const length = GodotRuntime.strlen(p_str) + 1
                  , c_str = GodotRuntime.malloc(length);
                return stringToUTF8(p_str, c_str, length),
                c_str
            },
            allocStringArray: function(p_strings) {
                const size = p_strings.length
                  , c_ptr = GodotRuntime.malloc(size * 4);
          ...
          , GodotConfig = {
            ...
            init_config: function(p_opts) {
                ...
                p_opts.focusCanvas && GodotConfig.canvas.focus()
            },
          ...
          , GodotFS = {
            ...
            init: function(persistentPaths) {
                ...
                return GodotFS._mount_points.forEach(function(path) {
                    createRecursive(path),
                    FS.mount(IDBFS, {}, path)
                }),
                new Promise(function(resolve) {
                    FS.syncfs(!0, function(err) {
                        err ? (GodotFS._mount_points = [],
                        GodotFS._idbfs = !1,
                        GodotRuntime.print(`IndexedDB not available: ${err.message}`)) : GodotFS._idbfs = !0,
                        resolve(err)
                    })
                }
                )
            ...
            deinit: function() {
                GodotFS._mount_points.forEach(function(path) {
                    ...
                    GodotFS._idbfs && IDBFS.dbs[path] && (IDBFS.dbs[path].close(),
            ...
            sync: function() {
                return GodotFS._syncing ? (GodotRuntime.error("Already syncing!"),
                Promise.resolve()) : (GodotFS._syncing = !0,
                new Promise(function(resolve) {
                    FS.syncfs(!1, function(error) {
                        error && GodotRuntime.error(`Failed to save IDB file system: ${error.message}`),
                        GodotFS._syncing = !1,
                        resolve(error)
                    })
                }
          ...
          , GodotOS = {
            ...
            finish_async: function(callback) {
                GodotOS._fs_sync_promise.then(function() {
                    const promises = [];
                    return GodotOS._async_cbs.forEach(function(cb) {
          ...
          , GodotAudio = {
            ...
            Sample: class Sample {
                ...
                static create(params, options={}) {
                    ...
                    return GodotAudio.samples.set(params.id, sample),
                    sample
                }
                static delete(id) {
            ...
            SampleNode: class SampleNode {
                ...
                static create(params, options={}) {
                    ...
                    return GodotAudio.sampleNodes.set(params.id, sampleNode),
                    sampleNode
                }
                static delete(id) {
                ...
                constructor(params, options={}) {
                    ...
                    const bus = GodotAudio.Bus.getBus(params.busIndex)
                      , sampleNodeBus = this.getSampleNodeBus(bus);
                ...
                async connectPositionWorklet(start) {
                    if (await GodotAudio.audioPositionWorkletPromise,
                    this.isCanceled)
                        return;
                ...
                getPositionWorklet() {
                    return this._positionWorklet != null ? this._positionWorklet : (this._positionWorklet = new AudioWorkletNode(GodotAudio.ctx,"godot-position-reporting-processor"),
                ...
                clear() {
                    ...
                    this.isPaused = !1,
                    this.pauseTime = 0,
                    this._source != null && (this._source.removeEventListener("ended", this._onended),
                    this._onended = null,
                    ...
                    this._positionWorklet && (this._positionWorklet.disconnect(),
                ...
                _restart() {
                    this._source != null && this._source.disconnect(),
                    this._source = GodotAudio.ctx.createBufferSource(),
                    this._source.buffer = this.getSample().getAudioBuffer();
                    for (const sampleNodeBus of this._sampleNodeBuses.values())
                    ...
                    this._positionWorklet != null && (this._positionWorklet.port.postMessage({
                        type: "clear"
                    }),
                ...
                _addEndedListener() {
                    this._onended != null && this._source.removeEventListener("ended", this._onended);
                    const self = this;
                    this._onended = _ => {
                        if (self.isPaused)
                            return;
            ...
            Bus: class Bus {
                ...
                static getBusOrNull(index) {
                    return index < 0 || index >= GodotAudio.buses.length ? null : GodotAudio.buses[index]
                }
                ...
                static addAt(index) {
                    ...
                    index !== newBus.getId() && GodotAudio.Bus.move(newBus.getId(), index)
                }
                static create() {
                    const newBus = new GodotAudio.Bus
                      , isFirstBus = GodotAudio.buses.length === 0;
                    return GodotAudio.buses.push(newBus),
                    isFirstBus ? newBus.setSend(null) : newBus.setSend(GodotAudio.Bus.getBus(0)),
                    newBus
                }
                constructor() {
                ...
                setVolumeDb(val) {
                    ...
                    isFinite(linear) && (this._gainNode.gain.value = linear)
                }
                getSend() {
                    return this._send
                }
                ...
                solo(enable) {
                    if (this.isSolo === enable)
                        return;
                    if (enable) {
                        GodotAudio.busSolo != null && GodotAudio.busSolo !== this && GodotAudio.busSolo._disableSolo(),
                        this._enableSolo();
                        return
                    }
                    this._disableSolo()
                ...
                connect(bus) {
                    ...
                    return this.getOutputNode().disconnect(),
            ...
            init: function(mix_rate, latency, onstatechange, onlatencyupdate) {
                ...
                GodotAudio.busSolo = null;
                const opts = {};
                mix_rate && (GodotAudio.sampleRate = mix_rate,
                ...
                GodotAudio.interval = setInterval(function() {
                    ...
                    ctx.baseLatency && (computed_latency += GodotAudio.ctx.baseLatency),
                    ctx.outputLatency && (computed_latency += GodotAudio.ctx.outputLatency),
                    onlatencyupdate(computed_latency)
                }, 1e3),
                GodotOS.atexit(GodotAudio.close_async);
                const path = GodotConfig.locate_file("godot.audio.position.worklet.js");
                return GodotAudio.audioPositionWorkletPromise = ctx.audioWorklet.addModule(path),
            ...
            create_input: function(callback) {
                ...
                if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia)
                    ...
                    !navigator.getUserMedia)
                        return GodotRuntime.error("getUserMedia not available."),
                        1;
                    navigator.getUserMedia({
                        audio: !0
            ...
            close_async: function(resolve) {
                ...
                GodotAudio.interval && (clearInterval(GodotAudio.interval),
                GodotAudio.interval = 0),
                GodotAudio.input && (GodotAudio.input.disconnect(),
                GodotAudio.input = null);
                let closed = Promise.resolve();
                GodotAudio.driver && (closed = GodotAudio.driver.close()),
            ...
            set_sample_bus_send: function(busIndex, sendIndex) {
                ...
                targetBus == null && (targetBus = GodotAudio.Bus.getBus(0)),
        ...
        function _godot_audio_get_sample_playback_position(playbackObjectIdStrPtr) {
            const playbackObjectId = GodotRuntime.parseString(playbackObjectIdStrPtr)
              , sampleNode = GodotAudio.SampleNode.getSampleNodeOrNull(playbackObjectId);
            return sampleNode == null ? 0 : sampleNode.getPlaybackPosition()
        }
        ...
        function _godot_audio_init(p_mix_rate, p_latency, p_state_change, p_latency_update) {
            const statechange = GodotRuntime.get_func(p_state_change)
              , latencyupdate = GodotRuntime.get_func(p_latency_update)
              , mix_rate = GodotRuntime.getHeapValue(p_mix_rate, "i32")
              , channels = GodotAudio.init(mix_rate, p_latency, statechange, latencyupdate);
            return GodotRuntime.setHeapValue(p_mix_rate, GodotAudio.ctx.sampleRate, "i32"),
            channels
        }
        ...
        function _godot_audio_resume() {
            GodotAudio.ctx && GodotAudio.ctx.state !== "running" && GodotAudio.ctx.resume()
        }
        ...
        function _godot_audio_sample_register_stream(streamObjectIdStrPtr, framesPtr, framesTotal, loopModeStrPtr, loopBegin, loopEnd) {
            const BYTES_PER_FLOAT32 = 4
              , streamObjectId = GodotRuntime.parseString(streamObjectIdStrPtr)
              , loopMode = GodotRuntime.parseString(loopModeStrPtr)
              , numberOfChannels = 2
              , sampleRate = GodotAudio.ctx.sampleRate
              , subLeft = GodotRuntime.heapSub(HEAPF32, framesPtr, framesTotal)
              , subRight = GodotRuntime.heapSub(HEAPF32, framesPtr + framesTotal * BYTES_PER_FLOAT32, framesTotal)
              , audioBuffer = GodotAudio.ctx.createBuffer(numberOfChannels, framesTotal, sampleRate);
        ...
        function _godot_audio_sample_set_volumes_linear(playbackObjectIdStrPtr, busesPtr, busesSize, volumesPtr, volumesSize) {
            const playbackObjectId = GodotRuntime.parseString(playbackObjectIdStrPtr)
              , buses = GodotRuntime.heapSub(HEAP32, busesPtr, busesSize)
              , volumes = GodotRuntime.heapSub(HEAPF32, volumesPtr, volumesSize);
        ...
        function _godot_audio_sample_start(playbackObjectIdStrPtr, streamObjectIdStrPtr, busIndex, offset, pitchScale, volumePtr) {
            const playbackObjectId = GodotRuntime.parseString(playbackObjectIdStrPtr)
              , streamObjectId = GodotRuntime.parseString(streamObjectIdStrPtr)
              , volume = GodotRuntime.heapSub(HEAPF32, volumePtr, 8)
              , startOptions = {
                offset,
                volume,
                playbackRate: 1,
        ...
        function _godot_audio_sample_unregister_stream(streamObjectIdStrPtr) {
            const streamObjectId = GodotRuntime.parseString(streamObjectIdStrPtr)
              , sample = GodotAudio.Sample.getSampleOrNull(streamObjectId);
            sample != null && sample.clear()
        }
        ...
        var GodotAudioScript = {
            ...
            create: function(buffer_length, channel_count) {
                return GodotAudioScript.script = GodotAudio.ctx.createScriptProcessor(buffer_length, 2, channel_count),
        ...
        function _godot_audio_script_create(buffer_length, channel_count) {
            ...
            try {
                ...
                return GodotRuntime.error("Error starting AudioDriverScriptProcessor", e),
                1
            }
            return 0
        }
        ...
        var GodotAudioWorklet = {
            ...
            create: function(channels) {
                ...
                GodotAudioWorklet.promise = GodotAudio.ctx.audioWorklet.addModule(path).then(function() {
                    return GodotAudioWorklet.worklet = new AudioWorkletNode(GodotAudio.ctx,"godot-processor",{
            ...
            start_no_threads: function(p_out_buf, p_out_size, out_callback, p_in_buf, p_in_size, in_callback) {
                function RingBuffer() {
                    ...
                    function send(port) {
                        ...
                        if (out_callback(wpos, pending_samples),
                        wpos + pending_samples >= size) {
                            const high = size - wpos;
                            wbuf.set(buffer.subarray(wpos, size)),
                            pending_samples -= high,
                            wpos = 0
                        }
                        pending_samples > 0 && wbuf.set(buffer.subarray(wpos, wpos + pending_samples), tot_sent - pending_samples),
                        port.postMessage({
                            cmd: "chunk",
                    ...
                    this.receive = function(recv_buf) {
                        ...
                        let to_write = recv_buf.length
                          , high = 0;
                        rpos + to_write >= p_in_size && (high = p_in_size - rpos,
                        buffer.set(recv_buf.subarray(0, high), rpos),
                        to_write -= high,
                        rpos = 0),
                        to_write && buffer.set(recv_buf.subarray(high, to_write), rpos),
                ...
                GodotAudioWorklet.promise.then(function() {
                    const node = GodotAudioWorklet.worklet
                      , buffer = GodotRuntime.heapSlice(HEAPF32, p_out_buf, p_out_size);
                    node.connect(GodotAudio.ctx.destination),
                    node.port.postMessage({
                        cmd: "start_nothreads",
        ...
        function _godot_audio_worklet_create(channels) {
            try {
                ...
                return GodotRuntime.error("Error starting AudioDriverWorklet", e),
                1
            }
            return 0
        }
        function _godot_audio_worklet_start_no_threads(p_out_buf, p_out_size, p_out_callback, p_in_buf, p_in_size, p_in_callback) {
            const out_callback = GodotRuntime.get_func(p_out_callback)
              , in_callback = GodotRuntime.get_func(p_in_callback);
        ...
        var GodotDisplayCursor = {
            ...
            set_shape: function(shape) {
                ...
                GodotDisplayCursor.visible && GodotDisplayCursor.set_style(css)
            },
            clear: function() {
          ...
          , _emscripten_webgl_do_get_current_context = () => GL.currentContext ? GL.currentContext.handle : 0
          , _emscripten_webgl_get_current_context = _emscripten_webgl_do_get_current_context
          , GodotDisplayScreen = {
            desired_size: [0, 0],
            hidpi: !0,
            ...
            isFullscreen: function() {
                ...
                return elem ? elem === GodotConfig.canvas : document.fullscreen || document.mozFullScreen || document.webkitIsFullscreen
            },
            ...
            _updateGL: function() {
                const gl_context_handle = _emscripten_webgl_get_current_context()
                  , gl = GL.getContext(gl_context_handle);
                gl && GL.resizeOffscreenFramebuffer(gl)
            },
            updateSize: function() {
                const isFullscreen = GodotDisplayScreen.isFullscreen()
                  , wantsFullWindow = GodotConfig.canvas_resize_policy === 2
                  , noResize = GodotConfig.canvas_resize_policy === 0
                  , dWidth = GodotDisplayScreen.desired_size[0]
                  , dHeight = GodotDisplayScreen.desired_size[1]
                  , canvas = GodotConfig.canvas;
                let width = dWidth
                  , height = dHeight;
                if (noResize)
                    return canvas.width !== width || canvas.height !== height ? (GodotDisplayScreen.desired_size = [canvas.width, canvas.height],
                    GodotDisplayScreen._updateGL(),
                    1) : 0;
                const scale = GodotDisplayScreen.getPixelRatio();
                (isFullscreen || wantsFullWindow) && (width = window.innerWidth * scale,
                height = window.innerHeight * scale);
                const csw = `${width / scale}px`
                  , csh = `${height / scale}px`;
                return canvas.style.width !== csw || canvas.style.height !== csh || canvas.width !== width || canvas.height !== height ? (canvas.width = width,
                canvas.height = height,
          ...
          , GodotDisplayVK = {
            ...
            init: function(input_cb) {
                function create(what) {
                    ...
                    elem.readonly = !0,
                    elem.disabled = !0,
                    GodotEventListeners.add(elem, "input", function() {
                        const c_str = GodotRuntime.allocString(elem.value);
                        input_cb(c_str, elem.selectionEnd),
                        GodotRuntime.free(c_str)
                    }, !1),
                    GodotEventListeners.add(elem, "blur", function() {
            ...
            show: function(text, type, start, end) {
                if (!GodotDisplayVK.textinput || !GodotDisplayVK.textarea)
                    return;
                (GodotDisplayVK.textinput.style.display !== "" || GodotDisplayVK.textarea.style.display !== "") && GodotDisplayVK.hide(),
                GodotDisplayVK.updateSize();
            ...
            clear: function() {
                GodotDisplayVK.textinput && (GodotDisplayVK.textinput.remove(),
                GodotDisplayVK.textinput = null),
                GodotDisplayVK.textarea && (GodotDisplayVK.textarea.remove(),
        ...
        function _godot_js_display_clipboard_set(p_text) {
            ...
            return !navigator.clipboard || !navigator.clipboard.writeText ? 1 : (navigator.clipboard.writeText(text).catch(function(e) {
        ...
        function _godot_js_display_cursor_lock_set(p_lock) {
            p_lock ? GodotDisplayCursor.lockPointer() : GodotDisplayCursor.releasePointer()
        }
        function _godot_js_display_cursor_set_custom_shape(p_shape, p_ptr, p_len, p_hotspot_x, p_hotspot_y) {
            const shape = GodotRuntime.parseString(p_shape)
              , old_shape = GodotDisplayCursor.cursors[shape];
            if (p_len > 0) {
            ...
            shape === GodotDisplayCursor.shape && GodotDisplayCursor.set_shape(GodotDisplayCursor.shape),
            old_shape && URL.revokeObjectURL(old_shape.url)
        }
        ...
        function _godot_js_display_cursor_set_visible(p_visible) {
            ...
            visible ? GodotDisplayCursor.set_shape(GodotDisplayCursor.shape) : GodotDisplayCursor.set_style("none")
        }
        ...
        function _godot_js_display_fullscreen_cb(callback) {
            const canvas = GodotConfig.canvas
              , func = GodotRuntime.get_func(callback);
            function change_cb(evt) {
                evt.target === canvas && func(GodotDisplayScreen.isFullscreen())
            }
            GodotEventListeners.add(document, "fullscreenchange", change_cb, !1),
            GodotEventListeners.add(document, "mozfullscreenchange", change_cb, !1),
            GodotEventListeners.add(document, "webkitfullscreenchange", change_cb, !1)
        }
        ...
        function _godot_js_display_setup_canvas(p_width, p_height, p_fullscreen, p_hidpi) {
            const canvas = GodotConfig.canvas;
            switch (GodotEventListeners.add(canvas, "contextmenu", function(ev) {
                ev.preventDefault()
            }, !1),
            ...
            GodotDisplayScreen.updateSize(),
            p_fullscreen && GodotDisplayScreen.requestFullscreen()
        }
        function _godot_js_display_size_update() {
            const updated = GodotDisplayScreen.updateSize();
            return updated && GodotDisplayVK.updateSize(),
            updated
        }
        ...
        function _godot_js_display_vk_cb(p_input_cb) {
            ...
            GodotDisplayVK.available() && GodotDisplayVK.init(input_cb)
        }
        ...
        function _godot_js_display_window_icon_set(p_ptr, p_len) {
            ...
            if (p_ptr) {
                link === null && (link = document.createElement("link"),
                link.rel = "icon",
            ...
            old_icon && URL.revokeObjectURL(old_icon)
        }
        ...
        function _godot_js_eval(p_js, p_use_global_ctx, p_union_ptr, p_byte_arr, p_byte_arr_write, p_callback) {
            ...
            case "boolean":
                return GodotRuntime.setHeapValue(p_union_ptr, eval_ret, "i32"),
                1;
            case "number":
                return GodotRuntime.setHeapValue(p_union_ptr, eval_ret, "double"),
                3;
            case "string":
                return GodotRuntime.setHeapValue(p_union_ptr, GodotRuntime.allocString(eval_ret), "*"),
                4;
            case "object":
                if (eval_ret === null)
                    break;
                if (ArrayBuffer.isView(eval_ret) && !(eval_ret instanceof Uint8Array) ? eval_ret = new Uint8Array(eval_ret.buffer) : eval_ret instanceof ArrayBuffer && (eval_ret = new Uint8Array(eval_ret)),
                eval_ret instanceof Uint8Array) {
                    const func = GodotRuntime.get_func(p_callback)
                      , bytes_ptr = func(p_byte_arr, p_byte_arr_write, eval_ret.length);
                    return HEAPU8.set(eval_ret, bytes_ptr),
                    29
                }
                break
            }
        ...
        var IDHandler = {
            ...
            add: function(p_data) {
                ...
                return IDHandler._references[id] = p_data,
                id
            },
          ...
          , GodotFetch = {
            onread: function(id, result) {
                ...
                if (!obj)
                    return;
                result.value && obj.chunks.push(result.value),
                obj.reading = !1,
            ...
            onresponse: function(id, response) {
                ...
                response.headers.forEach(function(value, header) {
                    const v = value.toLowerCase().trim()
                      , h = header.toLowerCase().trim();
                    h === "transfer-encoding" && v === "chunked" && (chunked = !0)
                }),
            ...
            create: function(method, url, headers, body) {
                const obj = {
                  ...
                  , id = IDHandler.add(obj)
                  , init = {
                    method,
                    headers,
                    body
                };
                return obj.request = fetch(url, init),
            ...
            read: function(id) {
                ...
                if (obj.reader && !obj.reading) {
                    ...
                    obj.reader == null && obj.response.body == null && (obj.reading = !0,
        ...
        function _godot_js_fetch_create(p_method, p_url, p_headers, p_headers_size, p_body, p_body_size) {
            const method = GodotRuntime.parseString(p_method)
              , url = GodotRuntime.parseString(p_url)
              , headers = GodotRuntime.parseStringArray(p_headers, p_headers_size)
              , body = p_body_size ? GodotRuntime.heapSlice(HEAP8, p_body, p_body_size) : null;
            return GodotFetch.create(method, url, headers.map(function(hv) {
                const idx = hv.indexOf(":");
                return idx <= 0 ? [] : [hv.slice(0, idx).trim(), hv.slice(idx + 1).trim()]
            }).filter(function(v) {
                return v.length === 2
            }), body)
        }
        ...
        function _godot_js_fetch_http_status_get(p_id) {
            ...
            return !obj || !obj.response ? 0 : obj.status
        }
        function _godot_js_fetch_is_chunked(p_id) {
            const obj = IDHandler.get(p_id);
            return !obj || !obj.response ? -1 : obj.chunked ? 1 : 0
        }
        function _godot_js_fetch_read_chunk(p_id, p_buf, p_buf_size) {
            ...
            for (; to_read && chunks.length; ) {
                const chunk = obj.chunks[0];
                chunk.length > to_read ? (GodotRuntime.heapCopy(HEAP8, chunk.slice(0, to_read), p_buf),
                chunks[0] = chunk.slice(to_read),
                to_read = 0) : (GodotRuntime.heapCopy(HEAP8, chunk, p_buf),
                to_read -= chunk.length,
                chunks.pop())
            }
            return chunks.length || GodotFetch.read(p_id),
            p_buf_size - to_read
        }
        function _godot_js_fetch_read_headers(p_id, p_parse_cb, p_ref) {
            ...
            return cb(arr.length, c_ptr, p_ref),
            GodotRuntime.freeStringArray(c_ptr, arr.length),
            0
        }
        function _godot_js_fetch_state_get(p_id) {
            const obj = IDHandler.get(p_id);
            return obj ? obj.error ? -1 : obj.response ? obj.reader || obj.response.body == null && !obj.done ? 1 : obj.done ? 2 : -1 : 0 : -1
        }
        var GodotInputGamepads = {
            ...
            init: function(onchange) {
                ...
                function add(pad) {
                    const guid = GodotInputGamepads.get_guid(pad)
                      , c_id = GodotRuntime.allocString(pad.id)
                      , c_guid = GodotRuntime.allocString(guid);
                ...
                GodotEventListeners.add(window, "gamepadconnected", function(evt) {
                    evt.gamepad && add(evt.gamepad)
                }, !1),
                GodotEventListeners.add(window, "gamepaddisconnected", function(evt) {
                    evt.gamepad && onchange(evt.gamepad.index, 0)
                }, !1)
            },
            get_guid: function(pad) {
                ...
                let os = "Unknown";
                ua.indexOf("Android") >= 0 ? os = "Android" : ua.indexOf("Linux") >= 0 ? os = "Linux" : ua.indexOf("iPhone") >= 0 ? os = "iOS" : ua.indexOf("Macintosh") >= 0 ? os = "MacOSX" : ua.indexOf("Windows") >= 0 && (os = "Windows");
                const id = pad.id
                  , exp1 = /vendor: ([0-9a-f]{4}) product: ([0-9a-f]{4})/i
                  , exp2 = /^([0-9a-f]+)-([0-9a-f]+)-/i;
                let vendor = ""
                  , product = "";
                if (exp1.test(id)) {
                ...
                return !vendor || !product ? `${os}Unknown` : os + vendor + product
            }
        }
          , GodotInputDragDrop = {
            promises: [],
            ...
            add_entry: function(entry) {
                entry.isDirectory ? GodotInputDragDrop.add_dir(entry) : entry.isFile ? GodotInputDragDrop.add_file(entry) : GodotRuntime.error("Unrecognized entry...", entry)
            },
            add_dir: function(entry) {
                GodotInputDragDrop.promises.push(new Promise(function(resolve) {
            ...
            _process_event: function(ev, callback) {
                ...
                ev.dataTransfer.items)
                    for (let i = 0; i < ev.dataTransfer.items.length; i++) {
                        ...
                        let entry = null;
                        "getAsEntry"in item ? entry = item.getAsEntry() : "webkitGetAsEntry"in item && (entry = item.webkitGetAsEntry()),
                        entry && GodotInputDragDrop.add_entry(entry)
                    }
                else
                ...
                new Promise(GodotInputDragDrop.process).then(function() {
                    ...
                    GodotInputDragDrop.pending_files.forEach(elem => {
                        ...
                        if (idx === -1)
                            ...
                            idx = sub.indexOf("/"),
                            idx < 0 && drops.indexOf(DROP + sub) === -1 && drops.push(DROP + sub)
                        }
                    ...
                    callback(drops),
                    GodotConfig.persistent_drops ? GodotOS.atexit(function(resolve) {
            ...
            remove_drop: function(files, drop_path) {
                ...
                files.forEach(function(file) {
                    ...
                    for (; idx > 0; )
                        ...
                        dirs.indexOf(drop_path + dir) === -1 && dirs.push(drop_path + dir),
                        idx = dir.lastIndexOf("/")
                }),
          ...
          , GodotIME = {
            ...
            ime_active: function(active) {
                ...
                if (GodotIME.focusTimerIntervalId > -1 && clearFocusTimerInterval(),
                GodotIME.ime == null)
                    return;
                GodotIME.active = active,
                active ? (GodotIME.ime.style.display = "block",
                GodotIME.focusTimerIntervalId = setInterval(focusTimer, 100)) : (GodotIME.ime.style.display = "none",
                GodotConfig.canvas.focus())
            },
            ...
            init: function(ime_cb, key_cb, code, key) {
                ...
                GodotEventListeners.add(ime, "compositionstart", ime_event_cb, !1),
                GodotEventListeners.add(ime, "compositionupdate", ime_event_cb, !1),
                GodotEventListeners.add(ime, "compositionend", ime_event_cb, !1),
                GodotEventListeners.add(ime, "keydown", key_event_cb.bind(null, 1), !1),
                GodotEventListeners.add(ime, "keyup", key_event_cb.bind(null, 0), !1),
                ime.onblur = function() {
            ...
            clear: function() {
                if (GodotIME.ime == null)
                    return;
                GodotIME.focusTimerIntervalId > -1 && (clearInterval(GodotIME.focusTimerIntervalId),
                GodotIME.focusTimerIntervalId = -1),
                GodotIME.ime.remove(),
                GodotIME.ime = null
            }
          ...
          , GodotInput = {
            ...
            computePosition: function(evt, rect) {
                const canvas = GodotConfig.canvas
                  , rw = canvas.width / rect.width
                  , rh = canvas.height / rect.height
                  , x = (evt.clientX - rect.x) * rw
                  , y = (evt.clientY - rect.y) * rh;
                return [x, y]
            }
        };
            ...
            const func = GodotRuntime.get_func(callback)
              , dropFiles = function(files) {
                ...
                const argc = args.length
                  , argv = GodotRuntime.allocStringArray(args);
                func(argv, argc),
        ...
        function _godot_js_input_gamepad_sample_get(p_index, r_btns, r_btns_num, r_axes, r_axes_num, r_standard) {
            ...
            const btns = sample.buttons
              , btns_len = btns.length < 16 ? btns.length : 16;
            ...
            const axes = sample.axes
              , axes_len = axes.length < 10 ? axes.length : 10;
            ...
            return GodotRuntime.setHeapValue(r_standard, is_standard, "i32"),
            0
        }
        function _godot_js_input_key_cb(callback, code, key) {
            ...
            GodotEventListeners.add(GodotConfig.canvas, "keydown", key_cb.bind(null, 1), !1),
            GodotEventListeners.add(GodotConfig.canvas, "keyup", key_cb.bind(null, 0), !1)
        }
        function _godot_js_input_mouse_button_cb(callback) {
            ...
            function button_cb(p_pressed, evt) {
                const rect = canvas.getBoundingClientRect()
                  , pos = GodotInput.computePosition(evt, rect)
                  , modifiers = GodotInput.getModifiers(evt);
                p_pressed && GodotConfig.canvas.focus(),
                func(p_pressed, evt.button, pos[0], pos[1], modifiers) && evt.preventDefault()
            }
            GodotEventListeners.add(canvas, "mousedown", button_cb.bind(null, 1), !1),
            GodotEventListeners.add(window, "mouseup", button_cb.bind(null, 0), !1)
        }
        function _godot_js_input_mouse_move_cb(callback) {
            ...
            function move_cb(evt) {
                const rect = canvas.getBoundingClientRect()
                  ...
                  , rw = canvas.width / rect.width
                  , rh = canvas.height / rect.height
                  , rel_pos_x = evt.movementX * rw
                  , rel_pos_y = evt.movementY * rh
                  , modifiers = GodotInput.getModifiers(evt);
                func(pos[0], pos[1], rel_pos_x, rel_pos_y, modifiers)
            }
            GodotEventListeners.add(window, "mousemove", move_cb, !1)
        }
        function _godot_js_input_mouse_wheel_cb(callback) {
            const func = GodotRuntime.get_func(callback);
            function wheel_cb(evt) {
                func(evt.deltaX || 0, evt.deltaY || 0) && evt.preventDefault()
            }
            GodotEventListeners.add(GodotConfig.canvas, "wheel", wheel_cb, !1)
        }
        function _godot_js_input_paste_cb(callback) {
            const func = GodotRuntime.get_func(callback);
            GodotEventListeners.add(window, "paste", function(evt) {
                const text = evt.clipboardData.getData("text")
                  , ptr = GodotRuntime.allocString(text);
                func(ptr),
                GodotRuntime.free(ptr)
            }, !1)
        }
        function _godot_js_input_touch_cb(callback, ids, coords) {
            ...
            function touch_cb(type, evt) {
                type === 0 && GodotConfig.canvas.focus();
                const rect = canvas.getBoundingClientRect()
                  , touches = evt.changedTouches;
                for (let i = 0; i < touches.length; i++) {
                    const touch = touches[i]
                      , pos = GodotInput.computePosition(touch, rect);
            ...
            GodotEventListeners.add(canvas, "touchstart", touch_cb.bind(null, 0), !1),
            GodotEventListeners.add(canvas, "touchend", touch_cb.bind(null, 1), !1),
            GodotEventListeners.add(canvas, "touchcancel", touch_cb.bind(null, 1), !1),
            GodotEventListeners.add(canvas, "touchmove", touch_cb.bind(null, 2), !1)
        }
        function _godot_js_input_vibrate_handheld(p_duration_ms) {
            typeof navigator.vibrate != "function" ? GodotRuntime.print("This browser does not support vibration.") : navigator.vibrate(p_duration_ms)
        }
        ...
        function _godot_js_os_download_buffer(p_ptr, p_size, p_name, p_mime) {
            const buf = GodotRuntime.heapSlice(HEAP8, p_ptr, p_size)
              , name = GodotRuntime.parseString(p_name)
              , mime = GodotRuntime.parseString(p_mime)
              , blob = new Blob([buf],{
                type: mime
            })
        ...
        function _godot_js_os_execute(p_json) {
            ...
            return GodotConfig.on_execute ? (GodotConfig.on_execute(args),
            0) : 1
        }
        ...
        function _godot_js_os_has_feature(p_ftr) {
            ...
            return ftr === "web_macos" ? ua.indexOf("Mac") !== -1 ? 1 : 0 : ftr === "web_windows" ? ua.indexOf("Windows") !== -1 ? 1 : 0 : ftr === "web_android" ? ua.indexOf("Android") !== -1 ? 1 : 0 : ftr === "web_ios" ? ua.indexOf("iPhone") !== -1 || ua.indexOf("iPad") !== -1 || ua.indexOf("iPod") !== -1 ? 1 : 0 : ftr === "web_linuxbsd" ? ua.indexOf("CrOS") !== -1 || ua.indexOf("BSD") !== -1 || ua.indexOf("Linux") !== -1 || ua.indexOf("X11") !== -1 ? 1 : 0 : 0
        }
        ...
        var GodotRTCDataChannel = {
            ...
            get_prop: function(p_id, p_prop, p_def) {
                const ref = IDHandler.get(p_id);
                return ref && ref[p_prop] !== 0[0] ? ref[p_prop] : p_def
            }
        };
        ...
        function _godot_js_rtc_datachannel_connect(p_id, p_ref, p_on_open, p_on_message, p_on_error, p_on_close) {
            const onopen = GodotRuntime.get_func(p_on_open).bind(null, p_ref)
              , onmessage = GodotRuntime.get_func(p_on_message).bind(null, p_ref)
              , onerror = GodotRuntime.get_func(p_on_error).bind(null, p_ref)
              , onclose = GodotRuntime.get_func(p_on_close).bind(null, p_ref);
        ...
        function _godot_js_rtc_datachannel_is_ordered(p_id) {
            return GodotRTCDataChannel.get_prop(p_id, "ordered", !0)
        }
        function _godot_js_rtc_datachannel_label_get(p_id) {
            const ref = IDHandler.get(p_id);
            return !ref || !ref.label ? 0 : GodotRuntime.allocString(ref.label)
        }
        function _godot_js_rtc_datachannel_max_packet_lifetime_get(p_id) {
            const ref = IDHandler.get(p_id);
            return ref ? ref.maxPacketLifeTime !== 0[0] ? ref.maxPacketLifeTime : ref.maxRetransmitTime !== 0[0] ? ref.maxRetransmitTime : 65535 : 65535
        }
        ...
        function _godot_js_rtc_datachannel_protocol_get(p_id) {
            const ref = IDHandler.get(p_id);
            return !ref || !ref.protocol ? 0 : GodotRuntime.allocString(ref.protocol)
        }
        ...
        var GodotRTCPeerConnection = {
            ...
            create: function(config, onConnectionChange, onSignalingChange, onIceGatheringChange, onIceCandidate, onDataChannel) {
                ...
                return "connectionState"in conn && conn.connectionState !== 0[0] ? conn.onconnectionstatechange = function() {
                    if (!IDHandler.get(id))
                        return;
                ...
                conn.onicecandidate = function(event) {
                    ...
                    const candidate_str = GodotRuntime.allocString(c.candidate)
                      , mid_str = GodotRuntime.allocString(c.sdpMid);
            ...
            onsession: function(p_id, callback, session) {
                ...
                const type_str = GodotRuntime.allocString(session.type)
                  , sdp_str = GodotRuntime.allocString(session.sdp);
        ...
        function _godot_js_rtc_pc_datachannel_create(p_id, p_label, p_config) {
            try {
                ...
                const label = GodotRuntime.parseString(p_label)
                  , config = JSON.parse(GodotRuntime.parseString(p_config))
                  , channel = ref.createDataChannel(label, config);
        ...
        function _godot_js_rtc_pc_ice_candidate_add(p_id, p_mid_name, p_mline_idx, p_sdp) {
            ...
            const sdpMidName = GodotRuntime.parseString(p_mid_name)
              , sdpName = GodotRuntime.parseString(p_sdp);
        ...
        function _godot_js_rtc_pc_local_description_set(p_id, p_type, p_sdp, p_obj, p_on_error) {
            ...
            const type = GodotRuntime.parseString(p_type)
              , sdp = GodotRuntime.parseString(p_sdp)
              , onerror = GodotRuntime.get_func(p_on_error).bind(null, p_obj);
        ...
        function _godot_js_rtc_pc_remote_description_set(p_id, p_type, p_sdp, p_obj, p_session_created, p_on_error) {
            ...
            const type = GodotRuntime.parseString(p_type)
              , sdp = GodotRuntime.parseString(p_sdp)
              , onerror = GodotRuntime.get_func(p_on_error).bind(null, p_obj)
              , onsession = GodotRuntime.get_func(p_session_created).bind(null, p_obj);
            ref.setRemoteDescription({
                sdp,
                type
            }).then(function() {
                return type !== "offer" ? Promise.resolve() : ref.createAnswer().then(function(session) {
                    GodotRTCPeerConnection.onsession(p_id, onsession, session)
                })
            }).catch(function(error) {
                GodotRTCPeerConnection.onerror(p_id, onerror, error)
        ...
        function _godot_js_set_ime_cb(p_ime_cb, p_key_cb, code, key) {
            const ime_cb = GodotRuntime.get_func(p_ime_cb)
              , key_cb = GodotRuntime.get_func(p_key_cb);
        ...
        function _godot_js_webmidi_open_midi_inputs(pSetInputNamesCb, pOnMidiMessageCb, pDataBuffer, dataBufferLen) {
            ...
            const setInputNamesCb = GodotRuntime.get_func(pSetInputNamesCb)
              , onMidiMessageCb = GodotRuntime.get_func(pOnMidiMessageCb);
            return GodotWebMidi.isListening = !0,
            navigator.requestMIDIAccess().then(midi => {
                const inputs = [...midi.inputs.values()]
                  , inputNames = inputs.map(input => input.name)
                  , c_ptr = GodotRuntime.allocStringArray(inputNames);
        ...
        var GodotWebSocket = {
            _onopen: function(p_id, callback) {
                const ref = IDHandler.get(p_id);
                if (!ref)
                    return;
            ...
            _onerror: function(p_id, callback) {
                const ref = IDHandler.get(p_id);
                if (!ref)
                    return;
                callback()
            ...
            send: function(p_id, p_data) {
                const ref = IDHandler.get(p_id);
                return !ref || ref.readyState !== ref.OPEN ? 1 : (ref.send(p_data),
                0)
            },
            bufferedAmount: function(p_id) {
                const ref = IDHandler.get(p_id);
                return ref ? ref.bufferedAmount : 0
            },
            create: function(socket, p_on_open, p_on_message, p_on_error, p_on_close) {
                const id = IDHandler.add(socket);
                return socket.onopen = GodotWebSocket._onopen.bind(null, id, p_on_open),
        ...
        function _godot_js_websocket_close(p_id, p_code, p_reason) {
            const code = p_code
              , reason = GodotRuntime.parseString(p_reason);
        ...
        function _godot_js_websocket_create(p_ref, p_url, p_proto, p_on_open, p_on_message, p_on_error, p_on_close) {
            const on_open = GodotRuntime.get_func(p_on_open).bind(null, p_ref)
              , on_message = GodotRuntime.get_func(p_on_message).bind(null, p_ref)
              , on_error = GodotRuntime.get_func(p_on_error).bind(null, p_ref)
              , on_close = GodotRuntime.get_func(p_on_close).bind(null, p_ref)
              , url = GodotRuntime.parseString(p_url)
              , protos = GodotRuntime.parseString(p_proto);
            let socket = null;
            try {
                protos ? socket = new WebSocket(url,protos.split(",")) : socket = new WebSocket(url)
            } catch {
                return 0
            }
            return socket.binaryType = "arraybuffer",
            GodotWebSocket.create(socket, on_open, on_message, on_error, on_close)
        }
        ...
        function _godot_js_websocket_send(p_id, p_buf, p_buf_len, p_raw) {
            ...
            return p_raw || (out = new TextDecoder("utf-8").decode(bytes_array)),
            GodotWebSocket.send(p_id, out)
        }
        var GodotJSWrapper = {
            proxies: null,
            cb_ret: null,
            ...
            get_proxied: function(val) {
                ...
                return IDHandler.get(id).ref(),
                id
            },
            get_proxied_value: function(id) {
                const proxy = IDHandler.get(id);
                return proxy === 0[0] ? 0[0] : proxy.get_val()
            },
            ...
            js2variant: function(p_val, p_exchange) {
                ...
                if (type === "boolean")
                    return GodotRuntime.setHeapValue(p_exchange, p_val, "i64"),
                    1;
                if (type === "number")
                    return Number.isInteger(p_val) ? (GodotRuntime.setHeapValue(p_exchange, p_val, "i64"),
                    2) : (GodotRuntime.setHeapValue(p_exchange, p_val, "double"),
                    3);
                if (type === "bigint")
                    return GodotRuntime.setHeapValue(p_exchange, p_val, "i64"),
                    2;
                if (type === "string") {
                    ...
                    return GodotRuntime.setHeapValue(p_exchange, c_str, "*"),
                    4
                }
                const id = GodotJSWrapper.get_proxied(p_val);
                return GodotRuntime.setHeapValue(p_exchange, id, "i64"),
                24
            },
        ...
        function _godot_js_wrapper_create_cb(p_ref, p_func) {
            ...
            const cb = function() {
                ...
                const args = Array.from(arguments)
                  , argsProxy = new GodotJSWrapper.MyProxy(args);
                func(p_ref, argsProxy.get_id(), args.length),
                argsProxy.unref();
                const ret = GodotJSWrapper.cb_ret;
                return GodotJSWrapper.cb_ret = null,
                ret
            };
            return id = GodotJSWrapper.get_proxied(cb),
            id
        }
        ...
        function _godot_js_wrapper_object_call(p_id, p_method, p_args, p_argc, p_convert_callback, p_exchange, p_lock, p_free_lock_callback) {
            ...
            if (obj === 0[0])
                return -1;
            const method = GodotRuntime.parseString(p_method)
              , convert = GodotRuntime.get_func(p_convert_callback)
              , freeLock = GodotRuntime.get_func(p_free_lock_callback)
              , args = new Array(p_argc);
            for (let i = 0; i < p_argc; i++) {
                const type = convert(p_args, i, p_exchange, p_lock)
            ...
            try {
                ...
                return GodotRuntime.error(`Error calling method ${method} on:`, obj, "error:", e),
                -1
            }
        }
        function _godot_js_wrapper_object_get(p_id, p_exchange, p_prop) {
            ...
            if (p_prop) {
                ...
                try {
                    ...
                    return GodotRuntime.error(`Error getting variable ${prop} on object`, obj),
                    0
                }
            }
        ...
        function _godot_js_wrapper_object_getvar(p_id, p_type, p_exchange) {
            ...
            try {
                ...
                return GodotRuntime.error(`Error getting variable ${prop} on object`, obj, e),
                -1
            }
        }
        ...
        function _godot_js_wrapper_object_setvar(p_id, p_key_type, p_key_ex, p_val_type, p_val_ex) {
            ...
            try {
                return obj[key] = GodotJSWrapper.variant2js(p_val_type, p_val_ex),
                0
            } catch {
                return GodotRuntime.error(`Error setting variable ${key} on object`, obj),
                -1
            }
        }
        function _godot_js_wrapper_object_transfer_buffer(p_id, p_byte_arr, p_byte_arr_write, p_callback) {
            ...
            if (!GodotJSWrapper.isBuffer(obj))
                return;
            ArrayBuffer.isView(obj) && !(obj instanceof Uint8Array) ? obj = new Uint8Array(obj.buffer) : obj instanceof ArrayBuffer && (obj = new Uint8Array(obj));
            const resizePackedByteArrayAndOpenWrite = GodotRuntime.get_func(p_callback)
              , bytesPtr = resizePackedByteArrayAndOpenWrite(p_byte_arr, p_byte_arr_write, obj.length);
            HEAPU8.set(obj, bytesPtr)
        }
        function _godot_js_wrapper_object_unref(p_id) {
            const proxy = IDHandler.get(p_id);
            proxy !== 0[0] && proxy.unref()
        }
        ...
        function _godot_webgl2_glGetBufferSubData(target, offset, size, data) {
            const gl_context_handle = _emscripten_webgl_get_current_context()
              , gl = GL.getContext(gl_context_handle);
            gl && gl.GLctx.getBufferSubData(target, offset, HEAPU8, data, size)
        }
        var GodotWebXR = {
            gl: null,
            session: null,
            ...
            monkeyPatchRequestAnimationFrame: enable => {
                GodotWebXR.orig_requestAnimationFrame === null && (GodotWebXR.orig_requestAnimationFrame = Browser.requestAnimationFrame),
            ...
            getLayer: () => {
                ...
                return layer = GodotWebXR.gl_binding.createProjectionLayer({
            ...
            getSubImage: () => {
                ...
                return layer === null ? null : GodotWebXR.gl_binding.getViewSubImage(layer, GodotWebXR.pose.views[0])
            }
            ,
            ...
            addInputSource: input_source => {
                ...
                if (name >= 0 && (GodotWebXR.input_sources[name] = input_source,
                input_source.name = name,
                input_source.targetRayMode === "screen")) {
                    let touch_index = -1;
                    for (let i = 0; i < 5; i++)
                    ...
                    touch_index >= 0 && (GodotWebXR.touches[touch_index] = input_source,
            ...
            removeInputSource: input_source => {
                if (input_source.name !== 0[0]) {
                    const name = input_source.name;
                    if (name >= 0 && name < 16 && (GodotWebXR.input_sources[name] = null),
                    input_source.touch_index !== 0[0]) {
                        const touch_index = input_source.touch_index;
                        touch_index >= 0 && touch_index < 5 && (GodotWebXR.touches[touch_index] = null)
                    }
                    return name
                }
                return -1
            ...
            ,
            getInputSourceId: input_source => input_source !== 0[0] ? input_source.name : -1,
            getTouchIndex: input_source => input_source.touch_index !== 0[0] ? input_source.touch_index : -1
        };
        function _godot_webxr_get_bounds_geometry(r_points) {
            ...
            return GodotRuntime.setHeapValue(r_points, buf, "i32"),
            point_count
        }
        function _godot_webxr_get_color_texture() {
            const subimage = GodotWebXR.getSubImage();
            return subimage === null ? 0 : GodotWebXR.getTextureId(subimage.colorTexture)
        }
        function _godot_webxr_get_depth_texture() {
            const subimage = GodotWebXR.getSubImage();
            return subimage === null ? 0 : subimage.depthStencilTexture ? GodotWebXR.getTextureId(subimage.depthStencilTexture) : 0
        }
        function _godot_webxr_get_frame_rate() {
            return !GodotWebXR.session || GodotWebXR.session.frameRate === 0[0] ? 0 : GodotWebXR.session.frameRate
        }
        ...
        function _godot_webxr_get_render_target_size(r_size) {
            const subimage = GodotWebXR.getSubImage();
            return subimage !== null && (GodotRuntime.setHeapValue(r_size + 0, subimage.viewport.width, "i32"),
        ...
        function _godot_webxr_get_supported_frame_rates(r_frame_rates) {
            if (!GodotWebXR.session || GodotWebXR.session.supportedFrameRates === 0[0])
                return 0;
            ...
            return GodotRuntime.setHeapValue(r_frame_rates, buf, "i32"),
            frame_rate_count
        }
        function _godot_webxr_get_transform_for_view(p_view, r_transform) {
            ...
            let matrix;
            p_view >= 0 ? matrix = views[p_view].transform.matrix : matrix = GodotWebXR.pose.transform.matrix;
            for (let i = 0; i < 16; i++)
                GodotRuntime.setHeapValue(r_transform + i * 4, matrix[i], "float");
            return !0
        }
        function _godot_webxr_get_velocity_texture() {
            const subimage = GodotWebXR.getSubImage();
            return subimage === null ? 0 : subimage.motionVectorTexture ? GodotWebXR.getTextureId(subimage.motionVectorTexture) : 0
        }
        ...
        function _godot_webxr_get_visibility_state() {
            return !GodotWebXR.session || !GodotWebXR.session.visibilityState ? 0 : GodotRuntime.allocString(GodotWebXR.session.visibilityState)
        }
        var _godot_webxr_initialize = function(p_session_mode, p_required_features, p_optional_features, p_requested_reference_spaces, p_on_session_started, p_on_session_ended, p_on_session_failed, p_on_input_event, p_on_simple_event) {
            ...
            const session_mode = GodotRuntime.parseString(p_session_mode)
              , required_features = GodotRuntime.parseString(p_required_features).split(",").map(s => s.trim()).filter(s => s !== "")
              , optional_features = GodotRuntime.parseString(p_optional_features).split(",").map(s => s.trim()).filter(s => s !== "")
              , requested_reference_space_types = GodotRuntime.parseString(p_requested_reference_spaces).split(",").map(s => s.trim())
              , onstarted = GodotRuntime.get_func(p_on_session_started)
              , onended = GodotRuntime.get_func(p_on_session_ended)
              , onfailed = GodotRuntime.get_func(p_on_session_failed)
              , oninputevent = GodotRuntime.get_func(p_on_input_event)
              , onsimpleevent = GodotRuntime.get_func(p_on_simple_event)
              , session_init = {};
            required_features.length > 0 && (session_init.requiredFeatures = required_features),
            optional_features.length > 0 && (session_init.optionalFeatures = optional_features),
            navigator.xr.requestSession(session_mode, session_init).then(function(session) {
                ...
                gl.makeXRCompatible().then(function() {
                    ...
                    function onReferenceSpaceSuccess(reference_space, reference_space_type) {
                        ...
                        reference_space.onreset = function() {
                        ...
                        window.setTimeout(function() {
                            const reference_space_c_str = GodotRuntime.allocString(reference_space_type)
                              , enabled_features = "enabledFeatures"in session ? Array.from(session.enabledFeatures) : []
                              , enabled_features_c_str = GodotRuntime.allocString(enabled_features.join(","))
                              , environment_blend_mode = "environmentBlendMode"in session ? session.environmentBlendMode : ""
                              , environment_blend_mode_c_str = GodotRuntime.allocString(environment_blend_mode);
        ...
        function _godot_webxr_is_session_supported(p_session_mode, p_callback) {
            const session_mode = GodotRuntime.parseString(p_session_mode)
              , cb = GodotRuntime.get_func(p_callback);
            if (navigator.xr)
        ...
        function _godot_webxr_update_input_source(p_input_source_id, r_target_pose, r_target_ray_mode, r_touch_index, r_has_grip_pose, r_grip_pose, r_has_standard_mapping, r_button_count, r_buttons, r_axes_count, r_axes, r_has_hand_data, r_hand_joints, r_hand_radii) {
            ...
            const input_source = GodotWebXR.input_sources[p_input_source_id]
              , frame = GodotWebXR.frame
              , space = GodotWebXR.space
              , target_pose = frame.getPose(input_source.targetRaySpace, space);
            if (!target_pose)
                return !1;
            ...
            let has_standard_mapping = !1
              , button_count = 0
              , axes_count = 0;
            if (input_source.gamepad) {
                input_source.gamepad.mapping === "xr-standard" && (has_standard_mapping = !0),
            ...
            if (input_source.hand && r_hand_joints !== 0 && r_hand_radii !== 0) {
                const hand_joint_array = new Float32Array(25 * 16)
                  , hand_radii_array = new Float32Array(25);
                frame.fillPoses(input_source.hand.values(), space, hand_joint_array) && frame.fillJointRadii(input_source.hand.values(), hand_radii_array) && (GodotRuntime.heapCopy(HEAPF32, hand_joint_array, r_hand_joints),
                GodotRuntime.heapCopy(HEAPF32, hand_radii_array, r_hand_radii),
                has_hand_data = !0)
            }
            return GodotRuntime.setHeapValue(r_has_hand_data, has_hand_data ? 1 : 0, "i32"),
            !0
        }
        function _godot_webxr_update_target_frame_rate(p_frame_rate) {
            if (!GodotWebXR.session || GodotWebXR.session.updateTargetFrameRate === 0[0])
                return;
        ...
        var stackAlloc = sz => __emscripten_stack_alloc(sz)
          , stringToUTF8OnStack = str => {
            var size = lengthBytesUTF8(str) + 1
              , ret = stackAlloc(size);
            return stringToUTF8(str, ret, size),
            ret
        }
          , getCFunc = ident => {
          ...
          , ccall = (ident, returnType, argTypes, args) => {
            toC = {
                string: str => {
                    var ret = 0;
                    return str != null && str !== 0 && (ret = stringToUTF8OnStack(str)),
                    ret
                }
                ,
                array: arr => {
                    var ret = stackAlloc(arr.length);
                    return writeArrayToMemory(arr, ret),
                    ret
                }
            };
            function convertReturnValue(ret) {
                return returnType === "string" ? UTF8ToString(ret) : returnType === "boolean" ? Boolean(ret) : ret
            }
            ...
            if (args)
                for (i = 0; i < args.length; i++) {
                    var toC, i, ret, converter = toC[argTypes[i]];
                    converter ? (stack === 0 && (stack = stackSave()),
                    cArgs[i] = converter(args[i])) : cArgs[i] = args[i]
                }
            ret = func(...cArgs);
            function onDone(ret) {
                return stack !== 0 && stackRestore(stack),
                convertReturnValue(ret)
            }
          ...
          , cwrap = (ident, returnType, argTypes, opts) => {
            ...
            return numericRet && numericArgs && !opts ? getCFunc(ident) : (...args) => ccall(ident, returnType, argTypes, args, opts)
        }
        ;
        ...
        GodotOS.atexit(function(resolve) {
        ...
        var wasmImports = {
          ...
          , ___wasm_call_ctors = () => (___wasm_call_ctors = wasmExports.jf)()
          , _free = a0 => (_free = wasmExports.kf)(a0)
          , __Z14godot_web_mainiPPc = Module.__Z14godot_web_mainiPPc = (a0, a1) => (__Z14godot_web_mainiPPc = Module.__Z14godot_web_mainiPPc = wasmExports.lf)(a0, a1)
          , _main = Module._main = (a0, a1) => (_main = Module._main = wasmExports.mf)(a0, a1)
          , _malloc = a0 => (_malloc = wasmExports.nf)(a0)
          , _fflush = a0 => (_fflush = wasmExports.of)(a0)
          , __emwebxr_on_input_event = Module.__emwebxr_on_input_event = (a0, a1) => (__emwebxr_on_input_event = Module.__emwebxr_on_input_event = wasmExports.pf)(a0, a1)
          , __emwebxr_on_simple_event = Module.__emwebxr_on_simple_event = a0 => (__emwebxr_on_simple_event = Module.__emwebxr_on_simple_event = wasmExports.qf)(a0)
          , ___funcs_on_exit = () => (___funcs_on_exit = wasmExports.sf)()
          , __emscripten_stack_restore = a0 => (__emscripten_stack_restore = wasmExports.tf)(a0)
          , __emscripten_stack_alloc = a0 => (__emscripten_stack_alloc = wasmExports.uf)(a0)
          , _emscripten_stack_get_current = () => (_emscripten_stack_get_current = wasmExports.vf)();
        ...
        function callMain(args=[]) {
            var ret, entryFunction = _main;
            args.unshift(thisProgram);
            var argc = args.length
              , argv = stackAlloc((argc + 1) * 4)
              , argv_ptr = argv;
            args.forEach(arg => {
            ...
            try {
                return ret = entryFunction(argc, argv),
                exitJS(ret, !0),
                ret
            } catch (e) {
...
typeof exports == "object" && typeof module == "object" ? module.exports = Godot : typeof define == "function" && define.amd && define([], () => Godot);
const Features = {
    ...
    isSecureContext: function() {
        return window.isSecureContext === !0
    },
    isCrossOriginIsolated: function() {
        return window.crossOriginIsolated === !0
    },
    ...
    getMissingFeatures: function(supportedFeatures={}) {
        const {threads: supportsThreads=!0} = supportedFeatures
          , missing = [];
        return Features.isWebGLAvailable(2) || missing.push("WebGL2 - Check web browser configuration and hardware support"),
        Features.isFetchAvailable() || missing.push("Fetch - Check web browser version"),
        Features.isSecureContext() || missing.push("Secure Context - Check web server configuration (use HTTPS)"),
        supportsThreads && (Features.isCrossOriginIsolated() || missing.push("Cross-Origin Isolation - Check that the web server configuration sends the correct headers."),
        Features.isSharedArrayBufferAvailable() || missing.push("SharedArrayBuffer - Check that the web server configuration sends the correct headers.")),
        missing
    }
}
  , Preloader = function() {
    function getTrackedResponse(response, load_status) {
        function onloadprogress(reader, controller) {
            return reader.read().then(function(result) {
                return load_status.done ? Promise.resolve() : (result.value && (controller.enqueue(result.value),
                load_status.loaded += result.value.length),
                result.done ? (load_status.done = !0,
                Promise.resolve()) : onloadprogress(reader, controller))
            })
        }
    ...
    function loadFetch(file, tracker, fileSize, raw) {
        ...
        fetch(file).then(function(response) {
            ...
            return Promise.resolve(tr.arrayBuffer().then(buffer => {
                try {
                    return new Response(pako.inflate(buffer))
                } catch (e) {
    ...
    function retry(func, attempts=1) {
        function onerror(err) {
            return attempts <= 1 ? Promise.reject(err) : new Promise(function(resolve, reject) {
                setTimeout(function() {
    ...
    const animateProgress = function() {
        ...
        Object.keys(loadingFiles).forEach(function(file) {
            ...
            stat.done || (progressIsFinal = !1),
            !totalIsValid || stat.total === 0 ? (totalIsValid = !1,
            total = 0) : total += stat.total,
            loaded += stat.loaded
        }),
        (loaded !== lastProgress.loaded || total !== lastProgress.total) && (lastProgress.loaded = loaded,
        lastProgress.total = total,
        typeof progressFunc == "function" && progressFunc(loaded, total)),
        progressIsFinal || requestAnimationFrame(animateProgress)
    };
    ...
    ,
    this.loadPromise = function(file, fileSize, raw=!1) {
    ...
    this.preload = function(pathOrBuffer, destPath, fileSize) {
        ...
        if (typeof pathOrBuffer == "string") {
            ...
            return this.loadPromise(pathOrBuffer, fileSize).then(function(buf) {
                return buf.arrayBuffer().then(buffer => {
        ...
        return pathOrBuffer instanceof ArrayBuffer ? buffer = new Uint8Array(pathOrBuffer) : ArrayBuffer.isView(pathOrBuffer) && (buffer = new Uint8Array(pathOrBuffer.buffer)),
        buffer ? (this.preloadedFiles.push({
            path: destPath,
            buffer: pathOrBuffer
        }),
        Promise.resolve()) : Promise.reject(new Error("Invalid object for preloading"))
    }
}
  , EngineConfig = {}
  , InternalConfig = function(initConfig) {
    const cfg = {
        unloadAfterInit: !0,
        canvas: null,
        executable: "",
    ...
    return Config.prototype = cfg,
    Config.prototype.update = function(opts) {
        const config = opts || {};
        function parse(key, def) {
            return typeof config[key] == "undefined" ? def : config[key]
        }
    ...
    Config.prototype.getModuleConfig = function(loadPath, response) {
        ...
        return {
            ...
            instantiateWasm: function(imports, onSuccess) {
                ...
                return typeof WebAssembly.instantiateStreaming != "undefined" ? WebAssembly.instantiateStreaming(Promise.resolve(r), imports).then(done) : r.arrayBuffer().then(function(buffer) {
            ...
            locateFile: function(path) {
                return path.startsWith("godot.") ? path.endsWith(".audio.worklet.js") ? `${loadPath}.audio.worklet.js` : path.endsWith(".audio.position.worklet.js") ? `${loadPath}.audio.position.worklet.js` : path.endsWith(".js") ? `${loadPath}.js` : path in gdext ? path : path.endsWith(".side.wasm") ? `${loadPath}.side.wasm` : path.endsWith(".wasm") ? `${loadPath}.wasm` : path : path
            }
        }
    }
    ,
    Config.prototype.getGodotConfig = function(cleanup) {
        ...
        return {
            ...
            onExit: function(p_code) {
                cleanup(),
                typeof onExit == "function" && onExit(p_code)
            }
        }
    }
    ,
  ...
  , Engine = function() {
    ...
    Engine.load = function(basePath, size) {
        return loadPromise == null && (loadPath = basePath,
        loadPromise = preloader.loadPromise(`${loadPath}.wasm`, size, !0),
    ...
    function SafeEngine(initConfig) {
        const proto = {
            init: function(basePath) {
                ...
                if (loadPromise == null) {
                    if (!basePath)
                        return initPromise = Promise.reject(new Error("A base path must be provided when calling `init` and the engine is not loaded.")),
                        initPromise;
                    ...
                    return new Promise(function(resolve) {
                        promise.then(function(response) {
                            ...
                            Godot(me.config.getModuleConfig(loadPath, cloned)).then(function(module) {
                                ...
                                module.initFS(paths).then(function() {
                                    me.rtenv = module,
                                    me.config.unloadAfterInit && Engine.unload(),
                                    resolve()
                                })
                            })
                        })
                ...
                return preloader.setProgressFunc(this.config.onProgress),
            ...
            start: function(override) {
                ...
                return me.init().then(function() {
                    ...
                    return me.rtenv.initConfig(config),
                    me.config.gdextensionLibs.length > 0 && !me.rtenv.loadDynamicLibrary ? Promise.reject(new Error('GDExtension libraries are not supported by this engine version. Enable "Extensions Support" for your export preset and/or build your custom template with "dlink_enabled=yes".')) : new Promise(function(resolve) {
            ...
            startGame: function(override) {
                ...
                const exe = this.config.executable
                  , pack = this.config.mainPack || `${exe}.pck`;
        ...
        new Engine(initConfig)
    }
    return SafeEngine.load = Engine.load,
